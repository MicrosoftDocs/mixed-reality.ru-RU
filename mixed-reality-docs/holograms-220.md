...
* Использование пространственного звука в сочетании с пространственным сопоставлением.
...
<td>Индекс MR 220: Пространственное звучание</td><td style="text-align: center;"> ✔️</td><td style="text-align: center;"> ✔️</td>
</tr>
</table>
...
>[!NOTE]
>Если вы хотите просмотреть исходный код перед загрузкой, он [доступен на сайте GitHub](https://github.com/Microsoft/HolographicAcademy/tree/Holograms-220-SpatialSound).
...
* На панели **проект** откройте **сценес\деЦибел.Унити**.
...
* В верхней части меню Unity выберите **правка > параметры проекта > аудио**.
...
4. В верхней строке меню щелкните Отладка **-> начать без отладки** или нажмите клавиши **CTRL + F5**. Если вы впервые развертываете на устройстве, вам потребуется [связать его с Visual Studio](using-visual-studio.md#pairing-your-device---hololens-1st-gen).
...
В Project шкала используется компонент **Аудиомиксер** Unity, позволяющий настраивать уровни звука для групп звуков. Группируя звуки таким образом, можно настроить общий том, сохраняя относительный объем каждого звука.

* В **аудиосаурце**разверните **Параметры трехмерного звука**.
...
Убедитесь, что на панели инструментов указано "выпуск", "x86" или "x64", а также "удаленное устройство". Если нет, это экземпляр кода Visual Studio. Возможно, потребуется повторно открыть решение из папки приложения.
...
Обратите внимание, что Аудиоемиттер обновляет шкалы времени, а не на основе каждого кадра. Это делается потому, что люди обычно не перемещаются достаточно быстро, чтобы их обновление выполнялось чаще, чем каждый квартал или половина секунды. Голограммы, которые можно быстро телепортируйтесь из одного места в другое, могут привести к нарушению иллюзии.
...
#### <a name="target-your-mix-for-70-volume-on-the-hololens"></a>Нацеливание на микшер для тома 70% на HoloLens
...
#### <a name="hololens-at-100-volume-should-drown-out-external-sounds"></a>На диске HoloLens на 100% Volume должны Drown внешние звуки
...
* Добавьте глубину в "волшебные окна".
* Перенесите пользователя в виртуальный мир.

### <a name="magic-windows"></a>Волшебные окна
...
* В **аудиосаурце**установите **выходные данные** в **UserVoice (микшер)** .
* Установите флажок **спатиализе** .
* Перетащите ползунок **пространственного смешения** на **3D**или введите **1** в поле ввода.
* Разверните **Параметры трехмерного звука**.
* Задайте для **уровня Doppler** значение **0**.
...
Поздравляем! Теперь вы завершили **MR 220: Пространственный звук**.
