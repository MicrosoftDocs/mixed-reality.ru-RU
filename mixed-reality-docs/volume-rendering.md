...
1. ПРАВИЛЬНЫМ Упрощенный подход: Отображение всего тома, обычно выполняется слишком медленно
2. РЕКОМЕНДУЕТСЯ Вырезание плоскости: Отображение только одного среза тома
3. РЕКОМЕНДУЕТСЯ Вырезание подразделов: Показывать только несколько слоев тома
4. РЕКОМЕНДУЕТСЯ Уменьшение разрешения отрисовки тома (см. раздел "Визуализация в режиме смешанного разрешения")
...
* 1028 * 720 * 2 * 1 = 1480320 (0,3% от полной) (тонкий срез: 1 выборка на пиксель, выполняется без проблем.)
* 1028 * 720 * 2 * 10 = 14803200 (3,9% от полной) (срез вспомогательного тома: 10 выборок на пиксель, выполняется довольно гладко, выглядит трехмерная)
* 200 * 200 * 2 * 256 = 20480000 (5% от полной) (более низкий объем ресурсов: меньше пикселей, полный том, объемный, но немного размытие)
