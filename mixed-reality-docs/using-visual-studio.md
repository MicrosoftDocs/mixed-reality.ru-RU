...
Независимо от того, хотите ли вы использовать DirectX или Unity для разработки приложения смешанной реальности, для отладки и развертывания будет использоваться Visual Studio. В этом разделе вы узнаете, как выполнять следующие задачи:
* Развертывайте приложения на основе HoloLens или иммерсивного головного телефона Windows Mixed Reality с помощью Visual Studio.
* Используйте эмулятор HoloLens, встроенный в Visual Studio.
...
Начните с включения **режима разработчика** на устройстве, чтобы в Visual Studio можно было подключиться к нему.
...
3. Для C++ проектов и JavaScript перейдите в **проект > свойства > Свойства конфигурации > отладки**. Для C# проектов будет автоматически отображаться диалоговое окно для настройки подключения.
...
3. Для C++ проектов и JavaScript перейдите в **проект > свойства > Свойства конфигурации > отладки**. Для C# проектов будет автоматически отображаться диалоговое окно для настройки подключения.
  а) Введите IP-адрес устройства в поле **адрес** или **имя компьютера** . Найдите IP-адрес в HoloLens в разделе **параметры > сеть & интернет > дополнительные параметры**или можно попросить Кортаны "что такое мой IP-адрес?".
  б. Задайте для режима проверки подлинности значение **универсальное (незашифрованный протокол)** ![диалоговом окне удаленного подключения в Visual Studio](images/remotedeploy.png)
4. Выберите **отладка > начать отладку** , чтобы развернуть приложение и начать отладку![запуск без отладки в Visual Studio](images/deploywithdebugging.png)
5. При первом развертывании приложения в HoloLens с компьютера будет предложено ввести ПИН-код. Следуйте приведенным ниже инструкциям по **связыванию с устройством** .
