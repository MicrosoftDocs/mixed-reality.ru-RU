...
2. Создайте новый проект универсального приложения Windows в Visual Studio 2015 с обновлением 1, Visual Studio 2017 или Visual Studio 2019. C#поддерживаются все проекты JavaScript, C++и. (Или следуйте инструкциям по [созданию сборки приложения в Unity](holograms-100.md).)
...
3. Взгляните на плитку **Параметры** и выполните жест [касания](gestures.md#air-tap). Выполните второе воздушное касание, чтобы разместить приложение в вашей среде. Приложение «Параметры» запустится после его размещения.
...
7. Дополнительно Прокрутите вниз, а также включите **портал устройств**. Это также позволит вам подключиться к [порталу устройств Windows](using-the-windows-device-portal.md) на HoloLens из веб-браузера.
...
1. Выберите конфигурацию сборки **x86** для конфигурации сборки x86 ![приложения в Visual Studio](images/x86setting.png)
2. Выберите **Удаленный компьютер** в раскрывающемся меню ![цель развертывания развертывание удаленный компьютер цель развертывания в Visual Studio.](images/remotemachinesetting.png)
3. Для C++ проектов и JavaScript перейдите в **проект > свойства > Свойства конфигурации > отладки**. Для C# проектов будет автоматически всплывающее диалоговое окно для настройки подключения.
  1\. Введите IP-адрес устройства в поле **адрес** или **имя компьютера** . Найдите IP-адрес в HoloLens в разделе **параметры > сеть & интернет > дополнительные параметры**или можно попросить Кортаны "что такое мой IP-адрес?".
  2\. Установка режима проверки подлинности в **универсальное (незашифрованное)** ![диалоговое окно удаленного подключения в Visual Studio](images/remotedeploy.png)
4. Выберите **Отладка > начать отладку** , чтобы развернуть приложение и![начать отладку без отладки в Visual Studio.](images/deploynodebugging.png)
...
1. Выберите конфигурацию сборки **x86** для конфигурации сборки x86 ![приложения в Visual Studio](images/x86setting.png)
2. Выберите **устройство** в раскрывающемся меню![целевого объекта развертывания развертывание устройства в Visual Studio](images/buildsettingsusbdeploy.png)
3. Выберите **Отладка > начать отладку** , чтобы развернуть приложение и![начать отладку без отладки в Visual Studio.](images/deploynodebugging.png)
...
## <a name="pairing-your-device---hololens-1st-gen"></a>Связывание устройства с HoloLens (1-й общий)
...
3. Выберите **эмулятор HoloLens** в раскрывающемся меню![целевого объекта развертывания цель эмулятора в Visual Studio.](images/deployemulator.png)
4. Выберите **Отладка > начать отладку** , чтобы развернуть приложение и![начать отладку без отладки в Visual Studio.](images/deploynodebugging.png)
