...
1.  Перейдите на [портал регистрации приложений](http://apps.dev.microsoft.com).
...
11. Обратите внимание, что таблица теперь содержит строку данных. Еще раз щелкните значок **(плюс),чтобыдобавитьдругуюсущность.+**
...
> [!WARNING]
> *Не изменяйте имя центра уведомлений напрямую* (все это должно быть выполнено с помощью **Расширенный редактор**, если выполнены предыдущие шаги правильно.
...
> [!NOTE] 
> Для этого курса **не** потребуется использовать контроллеры движения. Если вам нужна поддержка настройки иммерсивного гарнитуры, перейдите по этой ссылке, [чтобы настроить Windows Mixed Reality](https://support.microsoft.com/en-au/help/4043101/windows-10-set-up-windows-mixed-reality).
...
4.  Затем перейдите в раздел**параметры сборки** **файлов** > и выберите **универсальная платформа Windows**, а затем нажмите кнопку **переключения платформы** , чтобы применить выбранные элементы.
...
5.  Несмотря на то, что все еще находятся в**параметрах сборки** **файлов** > , убедитесь, что:
    ...
    6.  Здесь стоит сохранить сцену и добавить ее в сборку.
        ...
        3. Откройте только что созданную папку **сцены** , а затем в поле **имя файла:** введите **\_NH Desktop\_сцены**и нажмите кнопку **сохранить**.
...
10. Сохраните сцену и **файл** > проекта**сохранить сцену/файл** > **сохранить проект**.
...
3.  Щелкните правой кнопкой мыши внутри папки Scripts и выберите команду **создать**  >   **C# скрипт**. Назовите сценарий **таблетосцене**.

    ![новый скрипт](images/AzureLabs-Lab8-68.png)
    ![c# таблетосцене переименование](images/AzureLabs-Lab8-69.png)
...
1.  Щелкните правой кнопкой мыши в папке Scripts и выберите **создать**, а затем — **Сценарий C\#** . Назовите сценарий **клаудсцене**
    ...
    ![новый скрипт](images/AzureLabs-Lab8-72.png)
    ![c# переименование клаудсцене](images/AzureLabs-Lab8-73.png)
...
3.  Откроется окно **проводника** , в котором будет предложено создать расположение для сборки. Создайте новую папку (щелкнув **создать папку** в левом верхнем углу) и назовите ее **Build**.
    ...
    1.  Откройте папку "новые **сборки** " и создайте другую папку (с помощью **новой папки** еще раз) и назовите **ее\_NH\_Desktop App**.

        ![имя папки NH_Desktop_App](images/AzureLabs-Lab8-78.png)

    2.  Выбрав **\_классическое\_приложение NH** . Щелкните **выбрать папку**. Построение проекта займет около минуты.
...
5.  Перейдите в раздел**параметры сборки** **файлов** > и убедитесь в том, что:
    ...
    6.  Здесь стоит сохранить сцену и добавить ее в сборку.
        ...
        3. Откройте только что созданную папку **сцены** , а затем в поле **имя файла:** введите **\_NH MR\_сцены**и нажмите кнопку **сохранить**.
...
1.  Добавьте файл. пакет unitypackage, скачанный из приведенных выше, в Unity с помощью команды меню**настраиваемый пакет** для**импорта** >  **активов** > .

2.  В появившемся окне **Импорт пакета Unity** можно выбрать все в разделе**хранилище**подключаемых **модулей** > .
...
1.  Щелкните правой кнопкой мыши в папке Scripts и выберите **создать**, а затем — **Сценарий C\#** . Назовите сценарий **нотификатионрецеивер**.
    ...
    ![Создание нового имени сценария](images/AzureLabs-Lab8-95.png)
    ![c#, нотификатионрецеивер](images/AzureLabs-Lab8-96.png)
...
4.  Откроется окно **проводника** , в котором будет предложено создать расположение для сборки. Создайте новую папку (щелкнув **создать папку** в левом верхнем углу) и назовите ее **Build**.
    ...
    1.  Откройте папку "новые **сборки** " и создайте другую папку (с помощью **новой папки** еще раз) и назовите ее **\_NH\_-приложение MR**.
