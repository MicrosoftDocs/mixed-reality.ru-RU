...
# <a name="mr-and-azure-308-cross-device-notifications"></a>MR и Azure 308: уведомления между устройствами
...
<td> MR и Azure 308: уведомления между устройствами</td><td style="text-align: center;"> ✔️</td><td style="text-align: center;"> ✔️</td>
</tr>
</table>
...
2.  Войдя в систему, щелкните New ( **создать** ) в левом верхнем углу, а затем найдите **Центр уведомлений**и нажмите клавишу ***Ввод***.
...
4.  После нажатия кнопки ***создать***:
    ...
    2.  Укажите **пространство имен** , которое вы сможете связать с этим приложением.
...
10. Панель справа изменится на отображение сведений о **службе таблиц** , где необходимо добавить новую таблицу. Для этого нажмите кнопку **+** **таблицу** в левом верхнем углу.
...
11. Обратите внимание, что таблица теперь содержит строку данных. Щелкните значок **+** (плюс) еще раз, чтобы добавить другую сущность.
...
2.  При открытии блокнота вставьте в него структуру JSON. После этого сохраните его на рабочем столе как **Project. JSON**. Важно, чтобы имя было правильным: Убедитесь, что оно **не имеет расширения txt** . Этот файл определяет библиотеки, которые будет использовать функция. Если вы использовали NuGet, он покажется вам знакомым.
...
21. Перейдите к и щелкните файл **Project. JSON** , созданный ранее в **блокноте** , а затем нажмите кнопку **Открыть** . Этот файл определяет библиотеки, которые будет использовать функция.
...
25. В центральной панели откроется файл **Function. JSON** , который необходимо заменить на следующий фрагмент кода. Это определяет создаваемую функцию и параметры, передаваемые в функцию.
...
5.  Несмотря на то что в **файле** > **параметры сборки**, убедитесь, что:
    ...
    6.  Здесь стоит сохранить сцену и добавить ее в сборку.
        ...
        3. Откройте только что созданную папку **сцен** , а затем в поле **имя файла:** введите **NH\_Desktop\_сцену**, а затем нажмите кнопку **сохранить**.
...
10. Сохраните сцену и **файл** проекта > **сохранить сцену или файл** > **сохранить проект**.
...
Чтобы импортировать пакет SDK в собственный проект, убедитесь, что вы скачали последнюю версию [ **. пакет unitypackage**](https://aka.ms/azstorage-unitysdk) с сайта GitHub. Затем выполните следующие действия.

1.  Добавьте **. пакет unitypackage** в Unity с помощью параметра **активы \> импорт пакета \> настраиваемый пакет** меню.

2.  В появившемся окне **Импорт пакета Unity** можно выбрать все в разделе * **подключаемый модуль* \> * хранилище * * *.  Снимите флажок все остальное, так как он не требуется для этого курса.
...
Первый скрипт, который необходимо создать, — это **таблетосцене**, который отвечает за:
...
1.  Щелкните правой кнопкой мыши папку **Asset** , расположенную на панели проект, и создайте **папку** > . Назовите папку **Scripts**.
...
3.  Щелкните правой кнопкой мыши в папке **Scripts** и выберите **создать** >  **C# скрипт**. Назовите сценарий **таблетосцене**.

    ![новый](images/AzureLabs-Lab8-68.png)
    скрипта c# ![переименование Таблетосцене](images/AzureLabs-Lab8-69.png)
...
1.  Щелкните правой кнопкой мыши в папке **Scripts** и выберите **Create**, **C\# скрипт**. Назовите сценарий **клаудсцене**
    
    ![новый](images/AzureLabs-Lab8-72.png)
    скрипта c# ![переименование Клаудсцене](images/AzureLabs-Lab8-73.png)
...
3.  Откроется окно **проводника** , в котором будет предложено создать расположение для сборки. Создайте новую папку (щелкнув **создать папку** в левом верхнем углу) и назовите ее **Build**.
    ...
    1.  Откройте папку "новые **сборки** " и создайте другую папку (с помощью **новой папки** еще раз) и назовите ее **NH\_Desktop\_приложение**.

        ![имя папки NH_Desktop_App](images/AzureLabs-Lab8-78.png)

    2.  Выбрав **приложение NH\_Desktop\_** . Щелкните **выбрать папку**. Построение проекта займет около минуты.
...
5.  Перейдите в раздел **файл** > **параметры сборки** и убедитесь в том, что:
    ...
    6.  Здесь стоит сохранить сцену и добавить ее в сборку.
        ...
        3. Откройте только что созданную папку **сцен** , а затем в поле **имя файла:** введите **NH\_MR\_сцену**, а затем нажмите кнопку **сохранить**.
...
Чтобы импортировать пакет SDK в собственный проект, убедитесь, что вы скачали последнюю версию [. пакет unitypackage](https://aka.ms/azstorage-unitysdk). Затем выполните следующие действия.

1.  Добавьте пакет unitypackage, скачанный из приведенных выше, в Unity с помощью пункта **активы** > **импортировать пакет** > **настраиваемый пакет** .

2.  В появившемся окне **Импорт пакета Unity** можно выбрать все в разделе **подключаемый модуль** > **хранилище**.
...
1.  Щелкните правой кнопкой мыши в папке **Scripts** и выберите **Create**, **C\# скрипт**. Назовите сценарий **нотификатионрецеивер**.

    ![создать новый скрипт c#](images/AzureLabs-Lab8-95.png)
    ![назовите его Нотификатионрецеивер](images/AzureLabs-Lab8-96.png)
...
2.  В меню **параметры сборки** убедитесь, что **проект C# Unity*** является тактовым (что позволит изменять скрипты в этом проекте после сборки).

3.  По завершении нажмите кнопку **Сборка**.
...
4.  Откроется окно **проводника** , в котором будет предложено создать расположение для сборки. Создайте новую папку (щелкнув **создать папку** в левом верхнем углу) и назовите ее **Build**.
    ...
    1.  Откройте папку "новые **сборки** " и создайте другую папку (с помощью **новой папки** еще раз) и назовите ее **NH\_MR\_приложении**.

        ![создать папку NH_MR_Apps](images/AzureLabs-Lab8-101.png)

    2.  Выбрав **приложение NH\_MR\_** . Щелкните **выбрать папку**. Построение проекта займет около минуты.
...
3.  Выберите вкладку ***Обзор*** и выполните поиск по запросу **WindowsAzure. Messaging. Managed**.
...
3.  Появится новое окно **с именем связать приложение с магазином Windows**. Нажмите **Далее**.
...
5.  Найдите **имя приложения Магазина** , созданное в начале этого руководства, и выберите его. Нажмите **Далее**.
