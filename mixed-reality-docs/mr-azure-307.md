...
# <a name="mr-and-azure-307-machine-learning"></a>MR и Azure 307: машинное обучение

![окончательный продукт — запуск](images/AzureLabs-Lab7-0.png)

В этом курсе вы узнаете, как добавить возможности Машинное обучение (машинного обучения) в приложение смешанной реальности с помощью Машинное обучение Azure Studio.

*Машинное обучение Azure Studio* — это служба Майкрософт, которая предоставляет разработчикам большое количество алгоритмов машинного обучения, которые могут помочь в вводе, выводе, подготовке и визуализации данных. После этих компонентов можно разработать эксперимент прогнозной аналитики, выполнить его итерацию и использовать для обучения модели. После обучения можно сделать модель рабочей в облаке Azure, чтобы она могла затем оценить новые данные. Дополнительные сведения см. на [странице машинное обучение Azure Studio](https://azure.microsoft.com/services/machine-learning-studio/).
...
1.  Предоставьте таблицу данных о продажах на портале *машинное обучение Azure Studio* и создайте алгоритм для прогнозирования будущих продаж популярных элементов.
...
<td> MR и Azure 307: машинное обучение</td><td style="text-align: center;"> ✔️</td><td style="text-align: center;"> ✔️</td>
</tr>
</table>
...
## <a name="chapter-2---the-azure-machine-learning-studio"></a>Глава 2-Машинное обучение Azure Studio
...
9.  Чтобы войти в Машинное обучение Studio, используйте кнопку **Вход** , расположенную в правом верхнем углу или в центре.

    ![Машинное обучение Azure Studio](images/AzureLabs-Lab7-10.png)


## <a name="chapter-3---the-machine-learning-studio-dataset-setup"></a>Глава 3-Машинное обучение Studio: Настройка набора данных
...
1.  Вернитесь на портал **машинное обучение Studio** и добавьте эту таблицу в качестве **набора данных** для вашего машинного обучения. Для этого нажмите кнопку **+ создать** в левом нижнем углу экрана.
...
3.  Отправьте новый **набор данных** , выполнив следующие действия.
    ...
    3.  В текстовом поле ниже введите **продуктстаблексв. csv** в качестве имени для набора данных (хотя он должен быть добавлен автоматически).
...
## <a name="chapter-4---the-machine-learning-studio-the-experiment"></a>Глава 4-Машинное обучение Studio: эксперимент
...
7.  На панели **эксперименты** слева разверните узел **машинное обучение** > **обучения**. Перетащите элемент **обучение модели** в холст эксперимента. Холст должен выглядеть так же, как показано ниже.

    ![Машинное обучение Studio: эксперимент](images/AzureLabs-Lab7-20.png)

8.  В ***левом нижнем*** углу элемента « **разбиение данных** » перетащите соединение в **верхнюю правую** часть элемента « **обучение модели** ». В модели обучения для обучения алгоритму будет использоваться первый 70%, разделенный на набор данных.
...
11. Вы собираетесь обучить алгоритм **логистической регрессии с многоклассией** для прогнозирования наиболее проданного **продукта** на основе часа дня и даты. В этом документе нет сведений о различных алгоритмах, предоставляемых Машинное обучение Azure Studio, но вы можете узнать больше на [странице машинное обучение Algorithm Памятка по](https://docs.microsoft.com/azure/machine-learning/studio/algorithm-cheat-sheet) .

12. На панели "элементы эксперимента" слева разверните узел **Машинное обучение** > **инициализировать модель** > **классификация**и перетащите элемент **логистической регрессии** на холст эксперимента.
...
14. В списке элементов эксперимента на панели слева разверните **Машинное обучение** > **оценки**и перетащите элемент **модель оценки** на холст.
...
29. Откроется новая веб-страница, в которой будут показаны инструкции и некоторые примеры структуры запроса, необходимой для Машинное обучение Studio. Скопируйте **URI запроса** , показанный на этой странице, в Блокнот.
...
1.  Откройте **Unity** и создайте новый проект Unity с именем **MR\_MachineLearning.** Убедитесь, что для типа проекта задано значение **3D**.

2.  При открытом Unity стоит проверить, что для **редактора скриптов** по умолчанию задано значение **Visual Studio**. Перейдите к разделу **изменение** **параметров** > , а затем в новом окне перейдите к разделу **Внешние инструменты**. Измените **Редактор внешних скриптов** на **Visual Studio 2017**. Закройте окно **настройки** .

3.  Затем перейдите в раздел **файл** > **параметры сборки** и переключите платформу на **универсальная платформа Windows**, нажав кнопку ***коммутатора платформы*** .
...
6.  Назад в **параметрах сборки** проект *Unity C#*  больше не является серым. Установите флажок рядом с этим. 

7.  Закройте окно параметры сборки.
...
5.  В папке **панели проекта** щелкните папку **сцены** и дважды щелкните сцену внутри (под названием **MR_MachineLearningScene**). Будет открыта сцена (см. изображение ниже). Если красные ромбы отсутствуют, просто нажмите кнопку **приспособлений** в правом верхнем углу **игровой панели**.
...
4.  Вернувшись в редактор Unity, убедитесь, что класс **шелфкипер** выглядит следующим образом:
    ...
    > 2. В **иерархии** разверните объект **времени показа** (щелкнув правой кнопкой мыши стрелку рядом с ним). Затем щелкните ***основную камеру*** в **иерархии**, чтобы **инспектор** покажет сведения о ней.
...
2.  Щелкните правой кнопкой мыши внутри папки, **Создайте**  **C# скрипт** > . Вызовите скрипт **продуктпредиктион**.
...
15. Сохраните сцену и **файл** проекта > **сохранить сцену или файл** > **сохранить проект**.
...
3.  Установите флажок **проекты Unity C#**  (это важно, так как он позволит изменять классы после завершения сборки).
...
2.  После открытия Visual Studio необходимо восстановить пакеты NuGet, которые можно выполнить, щелкнув правой кнопкой мыши решение MachineLearningLab_Build в обозреватель решений (Найдено справа от Visual Studio), а затем выбрать команду восстановить пакеты NuGet:
