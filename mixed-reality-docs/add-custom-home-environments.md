...
---
# <a name="add-custom-home-environments"></a>Добавление пользовательских домашних сред
...
1. [Загрузите наш пример среды фэнтези](https://download.microsoft.com/download/B/2/5/B25C1AEF-40CD-4B03-A596-4BCA3D33035A/Fantasy_Island.exe) (ссылки указывают на самостоятельное извлечение исполняемого файла).
...
2. Запустите только что скачанный файл **Fantasy_Island. exe** .
    ...
    > [!NOTE]
    > При попытке запустить exe-файл, скачанный из Интернета (например, этот), может появиться всплывающее окно «защищенный компьютер Windows». Чтобы запустить Fantasy_Island. exe из этого всплывающего окна, выберите **Дополнительные сведения** и **запустите все равно**. Этот параметр безопасности предназначен для защиты от загрузки файлов, которые не следует доверять, поэтому выбирайте этот параметр только в том случае, если вы доверяете источнику файла.
