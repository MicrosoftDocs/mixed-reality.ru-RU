# <a name="mixing-color-and-depth-data"></a>Смешивание цвет и глубина данных

Ресурсы, полученной от камеры цвет и глубина камера у различных форм и размеров! Если требуется объединить два, есть несколько вещей, которые можно сделать, чтобы выровнять их друг с другом.
...
[Создание сетки из облака точки](#Creating-a-Mesh-From-the-Point-Cloud)  
[Создание образа на основе глубины и цвет](#Creating-an-Image-From-Depth-and-Color)  
[Очистка](#Cleaning-Up)  
...
Хотя мы могли бы использовать практически любую конфигурацию, которую нам нужно было здесь, мы свое низких разрешениях, так как мы начну несжатых данных в файл. Проверка документов на чтение из [камеры цвет]() и [камеры глубина]() Дополнительные сведения о конфигурациях!

Является еще одним элементом здесь `synchronized_images_only` параметр! Задание значения `true` указывает, что нам нужен только записей, имеющих оба цвет <i>и</i> глубины заполнен. Если мы не используем это, несколькими фиксирует из `k4a_device_get_capture` может либо недостает или цветных изображений.
...
Теперь, когда у нас есть наши сведения о конфигурации, необходимо настроить объект калибровки, будут переданы все наши функции преобразования! Этот объект содержит предварительно вычисленные ресурсы, используемые для преобразования координат в и из цветового пространства камеры на место и visa глубины. Что делает одной довольно прост:
...
Теперь нам нужно получить образ цвет и изображение глубины! Так что здесь получение один кадр и соответствующие данные из него. Если изображения поступают обратно в значение null, убедитесь, вы использовали `synchronized_images_only` в конфигурацию!
...
## <a name="transforming-data"></a>Преобразование данных

`k4a_transform_` Функции преобразует данные изображения из одного камеры пространства в другое! Итоговые образы будет соответствовать искажение и способы их устранения целевого пространства камеры, может помочь сопоставить цвет и глубина сведения друг с другом без труда.

Например! Довольно часто вы можете цветное изображение с камеры, соответствующий глубины для каждого цвета пикселя! Для этого будет преобразовывать глубины в цветовом пространстве камеры.

Кроме того может потребоваться создать облако трехмерной точки данных от камеры, глубины и знать, какой цвет каждой точки. Для этого мы преобразования данных глубины в облачной точке и преобразования в пространство камеры глубины цвета изображения! Получить буфер значения XYZ и соответствующего буфера цветов.

### <a name="depth-data-in-color-space"></a>Глубины в цветовом пространстве

Этот алгоритм преобразует глубины в соответствии с выходной цвет камеры! Получившимся буфером будет иметь значение глубины 16-разрядных систем для каждого пикселя в изображении цвет.
...
### <a name="creating-a-point-cloud"></a>Создание точки облака

Точка облака, также сохраняются в графических ресурсов! Здесь мы создадим образ формат `K4A_IMAGE_FORMAT_CUSTOM`, поскольку точка облака точно не является изображением. Каждый «» из пикселей изображение будет состоять из 3 `int16_t` значения, представляющие координаты XYZ, поэтому мы также указать правильный размер для этого.
...
### <a name="color-data-in-depth-space"></a>Данные о цвете в пространстве глубины

Этот алгоритм преобразует данных о цвете для сопоставления выходных данных от камеры, глубина! Каждое значение цвета с получившимся буфером будет соответствовать со значением из образа глубины.
...
## <a name="creating-a-mesh-from-the-point-cloud"></a>Создание сетки из облака точки

![](img/ColorandDepthMeshSmall.png)

Мы начнем с преобразованием данных облачной точки в формате, который немного более подробно ознакомиться. Мы конвертировать в миллиметрах метров и переместить его в числа с плавающей запятой.
...
Теперь мы будем совместить вверх этими вершинами с некоторой информацией лиц! Так как все, что выводится как сетка, это простой процесс связывания вверх углов каждой ячейки. Только дополнительные, что мы здесь делаем пропускает любое лицо, содержащий пустые данные. Если Kinect не удалось найти данные глубины точки в облаке точки, он помещает его в < 0,0,0 >! Становится блочным довольно hedgehog, при добавлении их в список индексов.
...
Затем мы просто можно записывать полученную информацию в файл! Мы не удаляя неиспользуемые цвета или позиций из-за дополнительными сложностями при создании индексов, поэтому можно просто передать в буферах, мы создали ранее.
...
## <a name="creating-an-image-from-depth-and-color"></a>Создание образа на основе глубины и цвет

![](img/ColorandDepthSmall.png)

С цветами это гораздо проще сделать! Здесь мы сделаем простой исчезания в черный цвет в расстояние, но удалось выполнить все виды интересных вещей, такие как обрезки out определенные диапазоны глубина или хранение глубины в альфа-канала для использования в качестве маски в средством работы с фотографиями!
...
## <a name="next-lab---accessing-the-imuusingimumd"></a>Далее лабораторное занятие — [доступ к IMU](UsingIMU.md)