...
<span data-ttu-id="c8b2b-142">Рекомендуется следующее оборудование и программное обеспечение для этого курса:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-142">We recommend the following hardware and software for this course:</span></span>
...
1.  <span data-ttu-id="c8b2b-155">Войдите в [портала Azure](https://portal.azure.com).</span><span class="sxs-lookup"><span data-stu-id="c8b2b-155">Log in to the  [Azure Portal](https://portal.azure.com).</span></span>
...
4.  <span data-ttu-id="c8b2b-164">Когда вы перейдете на **создать**:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-164">Once you have clicked on **Create**:</span></span>
    ...
    5.  <span data-ttu-id="c8b2b-171">Для *репликации* выберите **Read-access геоизбыточного хранилища (RA-GRS)**.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-171">For *Replication* select **Read-access-geo-redundant storage (RA-GRS)**.</span></span>
...
3.  <span data-ttu-id="c8b2b-204">Когда вы перейдете на **создать**:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-204">Once you have clicked on **Create**:</span></span>
    ...
    7.  <span data-ttu-id="c8b2b-218">Для *хранения*выберите **использовать существующий**, и затем с помощью раскрывающегося списка, найти ранее созданного хранилища.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-218">For *Storage*, select **Use existing**, and then using the dropdown menu, find your previously created storage.</span></span>
...
<span data-ttu-id="c8b2b-246">Следующие запущена типичный набор для разработки с помощью смешанной реальности и, таким образом, — это хороший шаблон для других проектов.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-246">The following is a typical set up for developing with Mixed Reality, and as such, is a good template for other projects.</span></span>
...
11. <span data-ttu-id="c8b2b-297">Сохраните сцену и проекта (**ФАЙЛ > Сохранить СЦЕНУ / FILE > Сохранить ПРОЕКТ**).</span><span class="sxs-lookup"><span data-stu-id="c8b2b-297">Save your Scene and Project (**FILE > SAVE SCENE / FILE > SAVE PROJECT**).</span></span>
...
4.  <span data-ttu-id="c8b2b-308">Затем обновите **преобразования** компонента, чтобы выглядеть следующим образом:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-308">Then update the **Transform** component to look like:</span></span>
    ...
    |         |    <span data-ttu-id="c8b2b-309">ПРЕОБРАЗОВАНИЕ - ПОЗИЦИИ</span><span class="sxs-lookup"><span data-stu-id="c8b2b-309">TRANSFORM - POSITION</span></span>   |       |
    | :-----: | :-----------------------: | :----:|
...
3.  <span data-ttu-id="c8b2b-351">Щелкните правой кнопкой мыши пустую часть области *панели иерархии*в разделе **трехмерный объект**, добавьте **куба**.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-351">Right-click in an empty area of the *Hierarchy Panel*, under **3D Object**, add a **Cube**.</span></span>
...
5.  <span data-ttu-id="c8b2b-371">Выберите новый объект и присвойте ему **ShapeSpawnPoint**.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-371">Select the new object and rename it **ShapeSpawnPoint**.</span></span>
    ...
    1.  <span data-ttu-id="c8b2b-372">Измените следующие параметры в *панели Инспектора*:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-372">Change the following parameters in the *Inspector Panel*:</span></span>
...
2.  <span data-ttu-id="c8b2b-417">В **Импорт пакета Unity** поле, отобразятся, вы сможете выбрать все данные \**подключаемый модуль* > \* хранилища \*\*.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-417">In the **Import Unity Package** box that pops up, you can select everything under \**Plugin* > \*Storage\*\*.</span></span> <span data-ttu-id="c8b2b-418">Снимите флажок, все остальное, так как он не требуется для этого курса.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-418">Uncheck everything else, as it is not needed for this course.</span></span>
...
2.  <span data-ttu-id="c8b2b-450">Дважды щелкните папку, только что создали, чтобы открыть его.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-450">Double click on the folder just created, to open it.</span></span>
...
<span data-ttu-id="c8b2b-471">Для создания этого класса:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-471">To create this class:</span></span>
...
6.  <span data-ttu-id="c8b2b-478">*CreateShape() позволяет* метод создает простые фигуры, основываясь на предоставленном *целое число* параметра.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-478">The *CreateShape()* method generates the primitive shapes, based upon the provided *integer* parameter.</span></span> <span data-ttu-id="c8b2b-479">Логический параметр, используемые для указания, является ли созданных фигуры из хранилище или новый.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-479">The Boolean parameter is used to specify whether the currently created shape is from storage, or new.</span></span> <span data-ttu-id="c8b2b-480">Поместите следующий код в ваш *ShapeFactory* класс ниже предыдущих методов:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-480">Place the following code in your *ShapeFactory* class, below the previous methods:</span></span>
...
7.  <span data-ttu-id="c8b2b-481">Не забудьте сохранить изменения в Visual Studio перед возвратом к Unity.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-481">Be sure to save your changes in Visual Studio before returning to Unity.</span></span>

8.  <span data-ttu-id="c8b2b-482">Обратно в редакторе Unity, щелкните и перетащите *ShapeFactory* класса из **сценарии** папку **Main Camera** объекта в *панели иерархии*.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-482">Back in the Unity Editor, click and drag the *ShapeFactory* class from the **Scripts** folder to the **Main Camera** object in the *Hierarchy Panel*.</span></span>

9. <span data-ttu-id="c8b2b-483">С Main Camera выбран можно заметить, *ShapeFactory* компонента скрипта отсутствует *Spawn точки* ссылки.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-483">With the Main Camera selected you will notice the *ShapeFactory* script component is missing the *Spawn Point* reference.</span></span> <span data-ttu-id="c8b2b-484">Чтобы устранить ее, перетащите **ShapeSpawnPoint** объекта из *панели иерархии* для **Spawn точки** цель ссылки.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-484">To fix it, drag the **ShapeSpawnPoint** object from the *Hierarchy Panel* to the **Spawn Point** reference target.</span></span>

    ![цель ссылки фабрики набора фигуры](images/AzureLabs-Lab5-53.png)

## <a name="chapter-9---create-the-gaze-class"></a><span data-ttu-id="c8b2b-486">Глава 9 — создать класс взглядом</span><span class="sxs-lookup"><span data-stu-id="c8b2b-486">Chapter 9 - Create the Gaze class</span></span>

<span data-ttu-id="c8b2b-487">— Последний скрипт, чтобы создать *помощи* класса.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-487">The last script you need to create is the *Gaze* class.</span></span>

<span data-ttu-id="c8b2b-488">Этот класс отвечает за создание **Raycast** , будет проецироваться вперед от Main Camera, для обнаружения какой объект, который просматривает пользователь.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-488">This class is responsible for creating a **Raycast** that will be projected forward from the Main Camera, to detect which object the user is looking at.</span></span> <span data-ttu-id="c8b2b-489">В этом случае Raycast будет необходимо определить, если пользователь просматривает **GazeButton** объект в сцене и активировать поведение.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-489">In this case, the Raycast will need to identify if the user is looking at the **GazeButton** object in the scene and trigger a behavior.</span></span>

<span data-ttu-id="c8b2b-490">Для создания данного класса:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-490">To create this Class:</span></span>

1.  <span data-ttu-id="c8b2b-491">Перейдите к **сценариев** папку, созданную ранее.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-491">Go to the **Scripts** folder you created previously.</span></span>

2.  <span data-ttu-id="c8b2b-492">Щелкните правой кнопкой мыши в панели «проект» **Создать > C# сценарий**.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-492">Right-click in the Project Panel, **Create > C# Script**.</span></span> <span data-ttu-id="c8b2b-493">Вызовите сценарий *помощи*.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-493">Call the script *Gaze*.</span></span>

3.  <span data-ttu-id="c8b2b-494">Дважды щелкните новый *помощи* скрипт, чтобы открыть его с *Visual Studio.*</span><span class="sxs-lookup"><span data-stu-id="c8b2b-494">Double click on the new *Gaze* script to open it with *Visual Studio.*</span></span>

4.  <span data-ttu-id="c8b2b-495">Убедитесь, что в верхней части скрипта включается следующее пространство имен:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-495">Ensure the following namespace is included at the top of the script:</span></span>

    ```csharp
        using UnityEngine;
    ```

5.  <span data-ttu-id="c8b2b-496">Затем добавьте следующие переменные внутри *помощи* класса:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-496">Then add the following variables inside the *Gaze* class:</span></span>
...
> [!IMPORTANT]
> <span data-ttu-id="c8b2b-497">Некоторые из этих переменных будут иметь возможность редактировать в *редактор*.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-497">Some of these variables will be able to be edited in the *Editor*.</span></span>

6.  <span data-ttu-id="c8b2b-498">Код для *Awake()* и *Start()* методы теперь должен быть добавлен.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-498">Code for the *Awake()* and *Start()* methods now needs to be added.</span></span>
...
7.  <span data-ttu-id="c8b2b-499">Добавьте следующий код, который создает объект курсора в начале, вместе с *Update()* метод, который будет выполнен метод Raycast, кроме того, где установлен GazeEnabled логическое значение:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-499">Add the following code, which will create a cursor object at start, along with the *Update()* method, which will run the Raycast method, along with being where the GazeEnabled boolean is toggled:</span></span>
...
8. <span data-ttu-id="c8b2b-500">Далее добавьте *UpdateRaycast()* метод, который будет проект Raycast и обнаружить попаданий целевой объект.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-500">Next add the *UpdateRaycast()* method, which will project a Raycast and detect the hit target.</span></span>
...
9. <span data-ttu-id="c8b2b-501">Наконец, добавьте *ResetFocusedObject()* метод, который будет составляться GazeButton объектов текущего цвета, указывающее, является ли он создает новую фигуру или нет.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-501">Lastly, add the *ResetFocusedObject()* method, which will toggle the GazeButton objects current color, indicating whether it is creating a new shape or not.</span></span>
...
10.  <span data-ttu-id="c8b2b-502">Сохраните изменения в Visual Studio перед возвратом к Unity.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-502">Save your changes in Visual Studio before returning to Unity.</span></span>

11.  <span data-ttu-id="c8b2b-503">Щелкните и перетащите *помощи* класс из папки скриптов для **Main Camera** объекта в *панели иерархии*.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-503">Click and drag the *Gaze* class from the Scripts folder to the **Main Camera** object in the *Hierarchy Panel*.</span></span>

## <a name="chapter-10---completing-the-azureservices-class"></a><span data-ttu-id="c8b2b-504">Глава 10 — завершение работы класса служб AzureServices</span><span class="sxs-lookup"><span data-stu-id="c8b2b-504">Chapter 10 - Completing the AzureServices class</span></span>

<span data-ttu-id="c8b2b-505">С помощью других сценариев на месте, он теперь имеется возможность *полный* *служб AzureServices* класса.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-505">With the other scripts in place, it is now possible to *complete* the *AzureServices* class.</span></span> <span data-ttu-id="c8b2b-506">Это будет осуществляться через:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-506">This will be achieved through:</span></span>

1.  <span data-ttu-id="c8b2b-507">Добавление нового метода с именем *CreateCloudIdentityAsync()*, чтобы задать переменные проверки подлинности, необходимые для взаимодействия с Azure.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-507">Adding a new method named *CreateCloudIdentityAsync()*, to set up the authentication variables needed for communicating with Azure.</span></span>

    > <span data-ttu-id="c8b2b-508">Этот метод также проверяет наличие ранее сохраненный файл, содержащий список фигуры.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-508">This method will also check for the existence of a previously stored File containing the Shape List.</span></span>
    >
    > <span data-ttu-id="c8b2b-509">**Если файл найден**, она происходит отключение пользователя *помощи*и запустить создание фигуры, согласно шаблону фигур, в том случае, как хранящиеся в **файла хранилища Azure**.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-509">**If the file is found**, it will disable the user *Gaze*, and trigger Shape creation, according to the pattern of shapes, as stored in the **Azure Storage file**.</span></span> <span data-ttu-id="c8b2b-510">Пользователь может видеть, как **Mesh текст** предоставит отображения «Хранилище» или «New», в зависимости от начала координат фигуры.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-510">The user can see this, as the **Text Mesh** will provide display 'Storage' or 'New', depending on the shapes origin.</span></span>
    >
    > <span data-ttu-id="c8b2b-511">**Если не найден файл**, это позволит *помощи*, позволяя пользователям для создания фигур, рассматривая **GazeButton** объект в сцене.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-511">**If no file is found**, it will enable the *Gaze*, enabling the user to create shapes when looking at the **GazeButton** object in the scene.</span></span>
...
2.  <span data-ttu-id="c8b2b-512">В следующем фрагменте кода является изнутри *Start()* метод; в случае, если будет выполнен вызов *CreateCloudIdentityAsync()* метод.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-512">The next code snippet is from within the *Start()* method; wherein a call will be made to the *CreateCloudIdentityAsync()* method.</span></span> <span data-ttu-id="c8b2b-513">Вы можете скопировать текущий *Start()* метод, с помощью ниже:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-513">Feel free to copy over your current *Start()* method, with the below:</span></span>
...
3.  <span data-ttu-id="c8b2b-514">Введите код для метода *CallAzureFunctionForNextShape()*.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-514">Fill in the code for the method *CallAzureFunctionForNextShape()*.</span></span> <span data-ttu-id="c8b2b-515">Вы будет использовать ранее созданный *приложение-функцию Azure* для запроса индекса фигуры.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-515">You will use the previously created *Azure Function App* to request a shape index.</span></span> <span data-ttu-id="c8b2b-516">После его получения, этот метод будет отправлять в фигуру, чтобы *ShapeFactory* класс, чтобы создать новую фигуру в сцену.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-516">Once the new shape is received, this method will send the shape to the *ShapeFactory* class to create the new shape in the scene.</span></span> <span data-ttu-id="c8b2b-517">Используйте приведенный ниже код для завершения тело *CallAzureFunctionForNextShape()*.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-517">Use the code below to complete the body of *CallAzureFunctionForNextShape()*.</span></span>
...
4.  <span data-ttu-id="c8b2b-518">Добавьте метод для создания строки, объединяя целых чисел, хранимых в журнале фигуры и помещения их в вашей *хранилища файлов Azure*.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-518">Add a method to create a string, by concatenating the integers stored in the shape history list, and saving it in your *Azure Storage File*.</span></span>
...
5.  <span data-ttu-id="c8b2b-519">Добавьте метод для извлечения текста, хранящегося в файле, расположенном в вашей *хранилища файлов Azure* и *десериализации* его в список.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-519">Add a method to retrieve the text stored in the file located in your *Azure Storage File* and *deserialize* it into a list.</span></span>

6.  <span data-ttu-id="c8b2b-520">После завершения этого процесса, метод повторно включает взглядом, таким образом, пользователь может добавлять дополнительные фигуры в сцену.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-520">Once this process is completed, the method re-enables the gaze so that the user can add more shapes to the scene.</span></span>
...
7.  <span data-ttu-id="c8b2b-521">Сохраните изменения в Visual Studio перед возвратом к Unity.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-521">Save your changes in Visual Studio before returning to Unity.</span></span>

## <a name="chapter-11---build-the-uwp-solution"></a><span data-ttu-id="c8b2b-522">Глава 11 - выполнить сборку решения универсальной платформы Windows</span><span class="sxs-lookup"><span data-stu-id="c8b2b-522">Chapter 11 - Build the UWP Solution</span></span>

<span data-ttu-id="c8b2b-523">Чтобы начать процесс построения:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-523">To begin the Build process:</span></span>

1.  <span data-ttu-id="c8b2b-524">Перейдите к **файл > Параметры сборки**.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-524">Go to **File > Build Settings**.</span></span>

    ![Построение приложения](images/AzureLabs-Lab5-54.png)

2.  <span data-ttu-id="c8b2b-526">Щелкните **Сборка**.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-526">Click **Build**.</span></span> <span data-ttu-id="c8b2b-527">Unity приведет к запуску *проводнике* окно, где необходимо создать, а затем выберите папку, чтобы создать приложение в.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-527">Unity will launch a *File Explorer* window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="c8b2b-528">Создайте эту папку и назовите его *приложения*.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-528">Create that folder now, and name it *App*.</span></span> <span data-ttu-id="c8b2b-529">Затем с помощью *приложения* папку, нажмите клавишу **Выбор папки**.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-529">Then with the *App* folder selected, press **Select Folder**.</span></span>

3.  <span data-ttu-id="c8b2b-530">Unity начнется построение проекта для *приложения* папки.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-530">Unity will begin building your project to the *App* folder.</span></span>

4.  <span data-ttu-id="c8b2b-531">Один раз Unity завершил построение (может занять некоторое время), он будет открыт *проводнике* окне в местоположении, построения (проверьте панели задач, так как он может не всегда отображаются над windows, но уведомит вас о Добавление нового окно).</span><span class="sxs-lookup"><span data-stu-id="c8b2b-531">Once Unity has finished building (it might take some time), it will open a *File Explorer* window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

## <a name="chapter-12---deploying-your-application"></a><span data-ttu-id="c8b2b-532">Глава 12 - развертывания приложения</span><span class="sxs-lookup"><span data-stu-id="c8b2b-532">Chapter 12 - Deploying your application</span></span>

<span data-ttu-id="c8b2b-533">Для развертывания приложения:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-533">To deploy your application:</span></span>

1.  <span data-ttu-id="c8b2b-534">Перейдите к *приложения* папку, которая была создана в [последняя глава](#chapter-11---build-the-uwp-solution).</span><span class="sxs-lookup"><span data-stu-id="c8b2b-534">Navigate to the *App* folder which was created in the [last Chapter](#chapter-11---build-the-uwp-solution).</span></span> <span data-ttu-id="c8b2b-535">Вы увидите файл с именем вашего приложения, с расширением «.sln», который следует дважды щелкнуть, таким образом, чтобы открыть его в *Visual Studio*.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-535">You will see a file with your apps name, with the '.sln' extension, which you should double-click, so to open it within *Visual Studio*.</span></span>

2.  <span data-ttu-id="c8b2b-536">В **платформа решения**выберите **x86, локальный компьютер**.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-536">In the **Solution Platform**, select **x86, Local Machine**.</span></span>

3.  <span data-ttu-id="c8b2b-537">В **конфигурации решения** выберите **Отладка**.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-537">In the **Solution Configuration** select **Debug**.</span></span>

    > <span data-ttu-id="c8b2b-538">Для Microsoft HoloLens, может быть проще устанавливать равным *удаленный компьютер*, таким образом, не связанном устройстве на компьютер.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-538">For the Microsoft HoloLens, you may find it easier to set this to *Remote Machine*, so that you are not tethered to your computer.</span></span> <span data-ttu-id="c8b2b-539">Однако необходимо будет выполнить следующее:</span><span class="sxs-lookup"><span data-stu-id="c8b2b-539">Though, you will need to also do the following:</span></span>
    > - <span data-ttu-id="c8b2b-540">Знать **IP-адрес** из HoloLens, которую можно найти в *параметры > сеть и Интернет > Wi-Fi > Дополнительно*; IPv4 — это адрес, следует использовать.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-540">Know the **IP Address** of your HoloLens, which can be found within the *Settings > Network & Internet > Wi-Fi > Advanced Options*; the IPv4 is the address you should use.</span></span> 
    > - <span data-ttu-id="c8b2b-541">Убедитесь, **режим разработчика** — **на**; найдено в *параметры > обновление и безопасность > для разработчиков*.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-541">Ensure **Developer Mode** is **On**; found in *Settings > Update & Security > For developers*.</span></span>

    ![Развертывание решения](images/AzureLabs-Lab5-55.png)

4.  <span data-ttu-id="c8b2b-543">Перейдите к **построения** меню и щелкните **развернуть решение** для загрузки неопубликованных приложений на компьютер.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-543">Go to the **Build** menu and click on **Deploy Solution** to sideload the application to your machine.</span></span>

5.  <span data-ttu-id="c8b2b-544">Приложения должны появиться в списке установленных приложений, все готово для запуска и протестировано!</span><span class="sxs-lookup"><span data-stu-id="c8b2b-544">Your App should now appear in the list of installed apps, ready to be launched and tested!</span></span>

## <a name="your-finished-azure-functions-and-storage-application"></a><span data-ttu-id="c8b2b-545">По завершении функций Azure и приложение службы хранилища</span><span class="sxs-lookup"><span data-stu-id="c8b2b-545">Your finished Azure Functions and Storage Application</span></span>

<span data-ttu-id="c8b2b-546">Поздравляем, вы создали приложение смешанной реальности, которое использует службы "функции Azure" и службы хранилища Azure.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-546">Congratulations, you built a mixed reality app that leverages both the Azure Functions and Azure Storage services.</span></span> <span data-ttu-id="c8b2b-547">Приложения будут иметь возможность рисования на сохраненных данных и укажите действие на основе этих данных.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-547">Your app will be able to draw on stored data, and provide an action based on that data.</span></span>

![конечный продукт-end](images/AzureLabs-Lab5-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="c8b2b-549">Упражнения премии</span><span class="sxs-lookup"><span data-stu-id="c8b2b-549">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="c8b2b-550">Упражнение 1</span><span class="sxs-lookup"><span data-stu-id="c8b2b-550">Exercise 1</span></span>

<span data-ttu-id="c8b2b-551">Создание второй spawn, точки и записи, которая порождает точки, для которого был создан объект.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-551">Create a second spawn point and record which spawn point an object was created from.</span></span> <span data-ttu-id="c8b2b-552">Когда вы загружаете файл данных, воспроизведение фигуры порожденных из расположения, в которых они изначально были созданы.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-552">When you load the data file, replay the shapes being spawned from the location they originally were created.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="c8b2b-553">Упражнение 2</span><span class="sxs-lookup"><span data-stu-id="c8b2b-553">Exercise 2</span></span>

<span data-ttu-id="c8b2b-554">Создайте способ перезапустить приложение, вместо того чтобы повторно открыть его каждый раз.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-554">Create a way to restart the app, rather than having to re-open it each time.</span></span> <span data-ttu-id="c8b2b-555">**Загрузка сцены** -подходящее место для запуска.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-555">**Loading Scenes** is a good spot to start.</span></span> <span data-ttu-id="c8b2b-556">После этого создайте очистки списка хранимых в *хранилища Azure*, так что можно легко восстановить ее из приложения.</span><span class="sxs-lookup"><span data-stu-id="c8b2b-556">After doing that, create a way to clear the stored list in *Azure Storage*, so that it can be easily reset from your app.</span></span> 
