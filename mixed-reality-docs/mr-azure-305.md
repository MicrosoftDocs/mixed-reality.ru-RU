...
# <a name="mr-and-azure-305-functions-and-storage"></a>MR и Azure 305: функции и хранилище
...
*Функции Azure* — это служба Майкрософт, которая позволяет разработчикам запускать небольшие фрагменты кода "функции" в Azure. Это позволяет делегировать работу в облако, а не локальное приложение, которое может иметь множество преимуществ. *Функции Azure* поддерживают несколько языков разработки, включая C\#, F\#, Node. js, Java и PHP. Дополнительные сведения см. в [статье о функциях Azure](https://docs.microsoft.com/azure/azure-functions/functions-overview).

*Хранилище Azure* — это облачная служба Майкрософт, которая позволяет разработчикам хранить данные с страховкой, которая будет высокодоступной, безопасной, устойчивой, масштабируемой и избыточной. Это означает, что корпорация Майкрософт будет выполнять все задачи обслуживания и критические проблемы. Дополнительные сведения см. в [статье о службе хранилища Azure](https://docs.microsoft.com/azure/storage/common/storage-introduction).
...
<td>MR и Azure 305: функции и хранилище</td><td style="text-align: center;"> ✔️</td><td style="text-align: center;"> ✔️</td>
</tr>
</table>
...
4.  После нажатия кнопки **создать**:
    ...
    6.  Для *повышения производительности*выберите **стандартный**.
...
2.  Теперь необходимо указать имя проекта Unity. Вставьте **MR_Azure_Functions**. Убедитесь, что для типа проекта задано значение **3D**. Задайте для *расположения нужное расположение* (Помните, что ближе к корневым каталогам лучше). Затем нажмите кнопку **создать проект**.
...
9.  Назад в *параметрах сборки* *проект Unity C#*  больше не является серым. Установите флажок рядом с этим.
...
6.  Далее предстоит создать **трехмерный текстовый** объект, чтобы оставить отзыв о состоянии службы Azure.
    ...
    Щелкните правой кнопкой мыши **газебуттон** на панели Иерархия и добавьте **трехмерный объект** > **трехмерный текстовый** объект в качестве *дочернего*.
...
Вы будете использовать службу хранилища Azure для Unity (которая самостоятельно использует пакет SDK для .NET для Azure). Дополнительные сведения об этом см. в [статье хранилище Azure для Unity](https://docs.microsoft.com/sandbox/gamedev/unity/azure-storage-unity).
...
Чтобы импортировать пакет SDK в собственный проект, убедитесь, что вы скачали последнюю версию [". пакет unitypackage" из GitHub](https://aka.ms/azstorage-unitysdk). Затем выполните следующие действия.

1.  Добавьте файл **. пакет unitypackage** в Unity с помощью пункта **активы** > **импортировать пакет** > **настраиваемый пакет** .
...
3.  Щелкните правой кнопкой мыши внутри папки, **Создайте**  **C# скрипт** > . Вызовите скрипт *азуресервицес*.
...
12. Выберите основную камеру, затем захватите дочерний объект **азурестатустекст** из объекта **газебуттон** и поместите его в поле Целевой объект ссылки **азурестатустекст** в *инспекторе*, чтобы предоставить ссылку на  *Сценарий Азуресервицес* .
...
2.  Щелкните правой кнопкой мыши внутри папки, **Создайте**  **C# скрипт** > . Вызовите скрипт *шапефактори*.
...
2.  Щелкните правой кнопкой мыши на панели проект, **Создайте**  **C# скрипт** > . Вызовите скрипт *взгляд*.
...
2.  Щелкните **Сборка**. Unity запустит окно *проводника* , в котором необходимо создать, а затем выбрать папку для сборки приложения. Создайте эту папку сейчас и назовите ее *app*Name. Затем выберите папку *приложения* и нажмите кнопку **выбрать папку**.
