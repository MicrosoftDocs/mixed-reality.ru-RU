...
## <a name="device-support"></a><span data-ttu-id="b5e5e-126">Поддержка устройств</span><span class="sxs-lookup"><span data-stu-id="b5e5e-126">Device support</span></span>
...
<span data-ttu-id="b5e5e-140">Для этого курса рекомендуется следующее оборудование и программное обеспечение:</span><span class="sxs-lookup"><span data-stu-id="b5e5e-140">We recommend the following hardware and software for this course:</span></span>
...
6.  <span data-ttu-id="b5e5e-264">Далее необходимо открыть папку **WSA** в папке **Newtonsoft**</span><span class="sxs-lookup"><span data-stu-id="b5e5e-264">Next, you need to open the **WSA** folder, within the **Newtonsoft** folder.</span></span> <span data-ttu-id="b5e5e-265">Вы увидите копию того же файла, который вы только что настроили.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-265">You will see a copy of the same file you just configured.</span></span> <span data-ttu-id="b5e5e-266">Выберите файл, а затем в инспекторе убедитесь, что</span><span class="sxs-lookup"><span data-stu-id="b5e5e-266">Select the file, and then in the inspector, ensure that</span></span>
...
1.  <span data-ttu-id="b5e5e-291">Щелкните правой кнопкой мыши *папку Asset* , расположенную на *панели проект*, и выберите команду **Создать > папку**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-291">Right-click in the *Asset Folder* located in the *Project Panel*, then click **Create > Folder**.</span></span> <span data-ttu-id="b5e5e-292">Вызовите **скрипты**папки.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-292">Call the folder **Scripts**.</span></span>
...
<span data-ttu-id="b5e5e-322">Чтобы создать этот класс, сделайте следующее:</span><span class="sxs-lookup"><span data-stu-id="b5e5e-322">To create this class:</span></span>
...
8.  <span data-ttu-id="b5e5e-331">Не забудьте сохранить изменения в **Visual Studio** перед возвратом в **Unity**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-331">Be sure to save your changes in **Visual Studio** before returning to **Unity**.</span></span>
...
10. <span data-ttu-id="b5e5e-361">Добавьте метод **траинкустомвисионпрожект ()** .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-361">Add the **TrainCustomVisionProject()** method.</span></span> <span data-ttu-id="b5e5e-362">После отправки и пометки образа будет вызван этот метод.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-362">Once the image has been submitted and tagged, this method will be called.</span></span> <span data-ttu-id="b5e5e-363">Он создаст новую итерацию, которая будет обучена всеми предыдущими изображениями, отправленными в службу, а также только что отправленное изображение.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-363">It will create a new Iteration that will be trained with all the previous images submitted to the Service plus the image just uploaded.</span></span> <span data-ttu-id="b5e5e-364">После завершения обучения этот метод вызывает метод, чтобы задать созданную итерацию **по умолчанию**, чтобы конечная точка, используемая для анализа, была последней обученной итерацией.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-364">Once the training has been completed, this method will call a method to set the newly created **Iteration** as **Default**, so that the endpoint you are using for analysis is the latest trained iteration.</span></span>
...
11. <span data-ttu-id="b5e5e-365">Добавьте метод **сетдефаултитератион ()** .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-365">Add the **SetDefaultIteration()** method.</span></span> <span data-ttu-id="b5e5e-366">Этот метод задаст ранее созданную и обученную итерацию *по умолчанию*.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-366">This method will set the previously created and trained iteration as *Default*.</span></span> <span data-ttu-id="b5e5e-367">После завершения этот метод должен удалить предыдущую итерацию, существующую в службе.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-367">Once completed, this method will have to delete the previous iteration existing in the Service.</span></span> <span data-ttu-id="b5e5e-368">На момент написания этого курса в службе можно одновременно использовать не более десяти итераций (10) одновременно.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-368">At the time of writing this course, there is a limit of a maximum ten (10) iterations allowed to exist at the same time in the Service.</span></span>
...
12. <span data-ttu-id="b5e5e-369">Добавьте метод **делетепревиауситератион ()** .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-369">Add the **DeletePreviousIteration()** method.</span></span> <span data-ttu-id="b5e5e-370">Этот метод найдет и удалит предыдущую нестандартную итерацию:</span><span class="sxs-lookup"><span data-stu-id="b5e5e-370">This method will find and delete the previous non-default iteration:</span></span>
...
13. <span data-ttu-id="b5e5e-371">Последний метод для добавления в этом классе — метод **жетимажеасбитеаррай ()** , используемый в веб-вызовах для преобразования изображения, захваченного в массив байтов.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-371">The last method to add in this class is the **GetImageAsByteArray()** method, used on the web calls to convert the image captured into a byte array.</span></span>
...
14. <span data-ttu-id="b5e5e-372">Не забудьте сохранить изменения в **Visual Studio** перед возвратом в **Unity**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-372">Be sure to save your changes in **Visual Studio** before returning to **Unity**.</span></span>

## <a name="chapter-10---create-the-sceneorganiser-class"></a><span data-ttu-id="b5e5e-373">Глава 10. Создание класса Сценеорганисер</span><span class="sxs-lookup"><span data-stu-id="b5e5e-373">Chapter 10 - Create the SceneOrganiser class</span></span>

<span data-ttu-id="b5e5e-374">Этот класс будет:</span><span class="sxs-lookup"><span data-stu-id="b5e5e-374">This class will:</span></span>

-   <span data-ttu-id="b5e5e-375">Создайте объект **курсора** для присоединения к основной камере.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-375">Create a **Cursor** object to attach to the Main Camera.</span></span>

-   <span data-ttu-id="b5e5e-376">Создайте объект **Label** , который будет отображаться, когда служба распознает реальные объекты.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-376">Create a **Label** object that will appear when the Service recognizes the real-world objects.</span></span>

-   <span data-ttu-id="b5e5e-377">Настройте основную камеру, присоединив к ней соответствующие компоненты.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-377">Set up the Main Camera by attaching the appropriate components to it.</span></span>

-   <span data-ttu-id="b5e5e-378">В **режиме анализа**, порождение меток во время выполнения, в соответствующем мировом пространстве относительно положения основной камеры и вывода данных, полученных от пользовательская служба визуального распознавания.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-378">When in **Analysis Mode**, spawn the Labels at runtime, in the appropriate world space relative to the position of the Main Camera, and display the data received from the Custom Vision Service.</span></span>

-   <span data-ttu-id="b5e5e-379">В **режиме обучения**Порождение пользовательского интерфейса, в котором будут отображаться различные этапы процесса обучения.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-379">When in **Training Mode**, spawn the UI that will display the different stages of the training process.</span></span>

<span data-ttu-id="b5e5e-380">Чтобы создать этот класс, сделайте следующее:</span><span class="sxs-lookup"><span data-stu-id="b5e5e-380">To create this class:</span></span>

1.  <span data-ttu-id="b5e5e-381">Щелкните правой кнопкой мыши в папке Scripts, а затем выберите команду **создать** > **Сценарий C\#** .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-381">Right-click inside the **Scripts** folder, then click **Create** > **C\# Script**.</span></span> <span data-ttu-id="b5e5e-382">Назовите сценарий *сценеорганисер*.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-382">Name the script *SceneOrganiser*.</span></span>

2.  <span data-ttu-id="b5e5e-383">Дважды щелкните новый скрипт *сценеорганисер* , чтобы открыть его в **Visual Studio**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-383">Double-click on the new *SceneOrganiser* script to open it with **Visual Studio**.</span></span>

3.  <span data-ttu-id="b5e5e-384">Вам потребуется только одно пространство имен, удалите остальные из класса *сценеорганисер* .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-384">You will only need one namespace, remove the others from above the *SceneOrganiser* class:</span></span>

    ```csharp
    using UnityEngine;
    ```

4.  <span data-ttu-id="b5e5e-385">Затем добавьте следующие переменные в класс *сценеорганисер* над методом **Start ()** :</span><span class="sxs-lookup"><span data-stu-id="b5e5e-385">Then add the following variables inside the *SceneOrganiser* class, above the **Start()** method:</span></span>
...
5.  <span data-ttu-id="b5e5e-386">Удалите методы **Start ()** и **Update ()** .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-386">Delete the **Start()** and **Update()** methods.</span></span>

6.  <span data-ttu-id="b5e5e-387">Непосредственно под переменными добавьте метод " **спящий ()** ", который инициализирует класс и настраивает сцену.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-387">Right underneath the variables, add the **Awake()** method, which will initialize the class and set up the scene.</span></span>
...
8. <span data-ttu-id="b5e5e-389">Добавьте метод **сеткамерастатус ()** , который будет работать с сообщениями, предназначенными для сетки текста, предоставляющей состояние камеры.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-389">Add the **SetCameraStatus()** method, which will handle messages intended for the text mesh providing the status of the camera.</span></span>
...
9. <span data-ttu-id="b5e5e-390">Добавьте методы **плацеаналисислабел ()** и **сеттагстоластлабел ()** , которые будут порождать и отображать данные из пользовательская служба визуального распознавания в сцене.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-390">Add the **PlaceAnalysisLabel()** and **SetTagsToLastLabel()** methods, which will spawn and display the data from the Custom Vision Service into the scene.</span></span>
...
10. <span data-ttu-id="b5e5e-391">Наконец, добавьте метод **креатетраинингуи ()** , который ПОРОЖДАЕТ пользовательский интерфейс, отображающий несколько стадий процесса обучения, когда приложение находится в режиме обучения.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-391">Lastly, add the **CreateTrainingUI()** method, which will spawn the UI displaying the multiple stages of the training process when the application is in Training Mode.</span></span> <span data-ttu-id="b5e5e-392">Этот метод также будет использоваться для создания объекта состояния камеры.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-392">This method will also be harnessed to create the camera status object.</span></span>
...
11. <span data-ttu-id="b5e5e-393">Не забудьте сохранить изменения в **Visual Studio** перед возвратом в **Unity**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-393">Be sure to save your changes in **Visual Studio** before returning to **Unity**.</span></span>

> [!IMPORTANT]
> <span data-ttu-id="b5e5e-394">Прежде чем продолжить, откройте класс **кустомвисионаналисер** и в методе **аналиселастимажекаптуред ()** раскомментируйте следующие строки:</span><span class="sxs-lookup"><span data-stu-id="b5e5e-394">Before you continue, open the **CustomVisionAnalyser** class, and within the **AnalyseLastImageCaptured()** method, *uncomment* the following lines:</span></span>
>
> ```csharp
>   AnalysisObject analysisObject = new AnalysisObject();
>   analysisObject = JsonConvert.DeserializeObject<AnalysisObject>(jsonResponse);
...
## <a name="chapter-11---create-the-imagecapture-class"></a><span data-ttu-id="b5e5e-395">Глава 11. Создание класса Имажекаптуре</span><span class="sxs-lookup"><span data-stu-id="b5e5e-395">Chapter 11 - Create the ImageCapture class</span></span>

<span data-ttu-id="b5e5e-396">Следующий класс, который предстоит создать, — это класс *имажекаптуре* .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-396">The next class you are going to create is the *ImageCapture* class.</span></span>

<span data-ttu-id="b5e5e-397">Этот класс отвечает за:</span><span class="sxs-lookup"><span data-stu-id="b5e5e-397">This class is responsible for:</span></span>

-   <span data-ttu-id="b5e5e-398">Запись изображения с помощью камеры HoloLens и сохранение его в папке *приложения* .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-398">Capturing an image using the HoloLens camera and storing it in the *App* Folder.</span></span>

-   <span data-ttu-id="b5e5e-399">Обработка жестов касания от пользователя.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-399">Handling Tap gestures from the user.</span></span>

-   <span data-ttu-id="b5e5e-400">Поддержание значения *перечисления* , определяющего, будет ли приложение запускаться в режиме *анализа* или *обучения* .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-400">Maintaining the *Enum* value that determines if the application will run in *Analysis* mode or *Training* Mode.</span></span>

<span data-ttu-id="b5e5e-401">Чтобы создать этот класс, сделайте следующее:</span><span class="sxs-lookup"><span data-stu-id="b5e5e-401">To create this class:</span></span>

1.  <span data-ttu-id="b5e5e-402">Перейдите к созданной ранее папке **Scripts** .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-402">Go to the **Scripts** folder you created previously.</span></span>

2.  <span data-ttu-id="b5e5e-403">Щелкните правой кнопкой мыши внутри папки и выберите команду **Создать > сценарий\# C**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-403">Right-click inside the folder, then click **Create > C\# Script**.</span></span> <span data-ttu-id="b5e5e-404">Назовите сценарий *имажекаптуре*.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-404">Name the script *ImageCapture*.</span></span>

3.  <span data-ttu-id="b5e5e-405">Дважды щелкните новый скрипт **имажекаптуре** , чтобы открыть его в **Visual Studio**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-405">Double-click on the new **ImageCapture** script to open it with **Visual Studio**.</span></span>

4.  <span data-ttu-id="b5e5e-406">Замените пространства имен в верхней части файла следующим:</span><span class="sxs-lookup"><span data-stu-id="b5e5e-406">Replace the namespaces at the top of the file with the following:</span></span>
...
5.  <span data-ttu-id="b5e5e-407">Затем добавьте следующие переменные в класс *имажекаптуре* над методом **Start ()** :</span><span class="sxs-lookup"><span data-stu-id="b5e5e-407">Then add the following variables inside the *ImageCapture* class, above the **Start()** method:</span></span>
...
6.  <span data-ttu-id="b5e5e-408">Теперь необходимо добавить код для методов **"спящий" ()** и **"начало" ()** :</span><span class="sxs-lookup"><span data-stu-id="b5e5e-408">Code for **Awake()** and **Start()** methods now needs to be added:</span></span>
...
7.  <span data-ttu-id="b5e5e-409">Реализуйте обработчик, который будет вызываться при возникновении жеста касания.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-409">Implement a handler that will be called when a Tap gesture occurs.</span></span>
    ...
    > [!NOTE] 
    > <span data-ttu-id="b5e5e-410">В режиме *анализа* метод **тафандлер** выступает в качестве параметра для запуска или завершения цикла записи фотографий.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-410">In *Analysis* mode, the **TapHandler** method acts as a switch to start or stop the photo capture loop.</span></span>
    >
    > <span data-ttu-id="b5e5e-411">В режиме *обучения* он будет записывать изображение с камеры.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-411">In *Training* mode, it will capture an image from the camera.</span></span>
    >
    > <span data-ttu-id="b5e5e-412">Когда курсор горит зеленым, это означает, что Камера доступна для получения образа.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-412">When the cursor is green, it means the camera is available to take the image.</span></span>
    >
    > <span data-ttu-id="b5e5e-413">Красный цвет курсора означает, что камера занята.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-413">When the cursor is red, it means the camera is busy.</span></span>

8.  <span data-ttu-id="b5e5e-414">Добавьте метод, используемый приложением для запуска процесса записи образа и сохранения образа.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-414">Add the method that the application uses to start the image capture process and store the image.</span></span>
...
9.  <span data-ttu-id="b5e5e-415">Добавьте обработчики, которые будут вызываться при записи фотографии и при ее готовности к анализу.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-415">Add the handlers that will be called when the photo has been captured and for when it is ready to be analyzed.</span></span> <span data-ttu-id="b5e5e-416">Затем результат передается в *кустомвисионаналисер* или *кустомвисионтраинер* в зависимости от того, в каком режиме установлен код.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-416">The result is then passed to the *CustomVisionAnalyser* or the *CustomVisionTrainer* depending on which mode the code is set on.</span></span>
...
10. <span data-ttu-id="b5e5e-417">Не забудьте сохранить изменения в **Visual Studio** перед возвратом в **Unity**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-417">Be sure to save your changes in **Visual Studio** before returning to **Unity**.</span></span>

11. <span data-ttu-id="b5e5e-418">Теперь, когда все сценарии завершены, вернитесь в редактор Unity, а затем перетащите класс **сценеорганисер** из папки Scripts в основной объект **Camera** на *панели Иерархия*.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-418">Now that all the scripts have been completed, go back in the Unity Editor, then click and drag the **SceneOrganiser** class from the **Scripts** folder to the **Main Camera** object in the *Hierarchy Panel*.</span></span>

## <a name="chapter-12---before-building"></a><span data-ttu-id="b5e5e-419">Глава 12-перед сборкой</span><span class="sxs-lookup"><span data-stu-id="b5e5e-419">Chapter 12 - Before building</span></span>

<span data-ttu-id="b5e5e-420">Чтобы выполнить тщательный тест приложения, необходимо загружать неопубликованные его на HoloLens.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-420">To perform a thorough test of your application you will need to sideload it onto your HoloLens.</span></span>

<span data-ttu-id="b5e5e-421">Перед этим убедитесь в том, что:</span><span class="sxs-lookup"><span data-stu-id="b5e5e-421">Before you do, ensure that:</span></span>

- <span data-ttu-id="b5e5e-422">Все параметры, упомянутые в [главе 2](#chapter-2---training-your-custom-vision-project) , заданы правильно.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-422">All the settings mentioned in the [Chapter 2](#chapter-2---training-your-custom-vision-project) are set correctly.</span></span>

- <span data-ttu-id="b5e5e-423">Все поля на **главной камере**, панели инспектора, назначаются должным образом.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-423">All the fields in the **Main Camera**, Inspector Panel, are assigned properly.</span></span>

- <span data-ttu-id="b5e5e-424">**Сценеорганисер** скрипта прикреплен к главному объекту **Camera** .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-424">The script **SceneOrganiser** is attached to the **Main Camera** object.</span></span>

- <span data-ttu-id="b5e5e-425">Убедитесь, что вы вставили **ключ прогноза** в переменную **предиктионкэй** .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-425">Make sure you insert your **Prediction Key** into the **predictionKey** variable.</span></span>

- <span data-ttu-id="b5e5e-426">Вы вставили **конечную точку прогнозирования** в переменную **предиктионендпоинт** .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-426">You have inserted your **Prediction Endpoint** into the **predictionEndpoint** variable.</span></span>

- <span data-ttu-id="b5e5e-427">Вы вставили **ключ обучения** в переменную **траинингкэй** класса *кустомвисионтраинер* .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-427">You have inserted your **Training Key** into the **trainingKey** variable, of the *CustomVisionTrainer* class.</span></span>

- <span data-ttu-id="b5e5e-428">Вы вставили **идентификатор проекта** в переменную **projectId** класса *кустомвисионтраинер* .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-428">You have inserted your **Project ID** into the **projectId** variable, of the *CustomVisionTrainer* class.</span></span>

## <a name="chapter-13---build-and-sideload-your-application"></a><span data-ttu-id="b5e5e-429">Глава 13. Создание и загружать неопубликованные приложения</span><span class="sxs-lookup"><span data-stu-id="b5e5e-429">Chapter 13 - Build and sideload your application</span></span>

<span data-ttu-id="b5e5e-430">Чтобы начать процесс *сборки* :</span><span class="sxs-lookup"><span data-stu-id="b5e5e-430">To begin the *Build* process:</span></span>

1.  <span data-ttu-id="b5e5e-431">Выберите **файл > параметры сборки**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-431">Go to **File > Build Settings**.</span></span>

2.  <span data-ttu-id="b5e5e-432">**Проекты тактов Unity\# C**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-432">Tick **Unity C\# Projects**.</span></span>

3.  <span data-ttu-id="b5e5e-433">Щелкните **Сборка**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-433">Click **Build**.</span></span> <span data-ttu-id="b5e5e-434">Unity запустит окно **проводника** , в котором необходимо создать, а затем выбрать папку для сборки приложения.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-434">Unity will launch a **File Explorer** window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="b5e5e-435">Создайте эту папку сейчас и назовите ее **app**Name.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-435">Create that folder now, and name it **App**.</span></span> <span data-ttu-id="b5e5e-436">Затем в выбранной папке **приложения** щелкните **выбрать папку**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-436">Then with the **App** folder selected, click on **Select Folder**.</span></span>

4.  <span data-ttu-id="b5e5e-437">Unity начнет сборку проекта в папку **приложения** .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-437">Unity will begin building your project to the **App** folder.</span></span>

5.  <span data-ttu-id="b5e5e-438">После того как Unity завершит сборку (может занять некоторое время), он откроет окно **проводника** в расположении сборки (проверьте панель задач, так как она может не всегда отображаться над окнами, но будет уведомлять о добавлении нового окна).</span><span class="sxs-lookup"><span data-stu-id="b5e5e-438">Once Unity has finished building (it might take some time), it will open a **File Explorer** window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

<span data-ttu-id="b5e5e-439">Для развертывания на HoloLens выполните следующие действия.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-439">To deploy on HoloLens:</span></span>

1.  <span data-ttu-id="b5e5e-440">Вам потребуется IP-адрес HoloLens (для удаленного развертывания) и убедитесь, что HoloLens находится в **режиме разработчика**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-440">You will need the IP Address of your HoloLens (for Remote Deploy), and to ensure your HoloLens is in **Developer Mode**.</span></span> <span data-ttu-id="b5e5e-441">Выполните указанные ниже действия.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-441">To do this:</span></span>

    1.  <span data-ttu-id="b5e5e-442">Людьми HoloLens, откройте **Параметры**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-442">Whilst wearing your HoloLens, open the **Settings**.</span></span>

    2.  <span data-ttu-id="b5e5e-443">Выберите сетевые &**Дополнительные параметры** **сети Интернет** > **Wi-Fi** > </span><span class="sxs-lookup"><span data-stu-id="b5e5e-443">Go to **Network & Internet** > **Wi-Fi** > **Advanced Options**</span></span>

    3.  <span data-ttu-id="b5e5e-444">Запишите **IPv4** -адрес.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-444">Note the **IPv4** address.</span></span>

    4.  <span data-ttu-id="b5e5e-445">Затем вернитесь к параметрами **Обновите & безопасности** > **для разработчиков** .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-445">Next, navigate back to **Settings**, and then to **Update & Security** > **For Developers**</span></span>

    5.  <span data-ttu-id="b5e5e-446">Задайте **режим разработчика на**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-446">Set **Developer Mode On**.</span></span>

2.  <span data-ttu-id="b5e5e-447">Перейдите к новой сборке Unity (папка **приложения** ) и откройте файл решения в **Visual Studio**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-447">Navigate to your new Unity build (the **App** folder) and open the solution file with **Visual Studio**.</span></span>

3.  <span data-ttu-id="b5e5e-448">В *конфигурации решения* выберите **Отладка**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-448">In the *Solution Configuration* select **Debug**.</span></span>

4.  <span data-ttu-id="b5e5e-449">На *платформе решения*выберите **x86, удаленный компьютер**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-449">In the *Solution Platform*, select **x86, Remote Machine**.</span></span> <span data-ttu-id="b5e5e-450">Вам будет предложено вставить **IP-адрес** удаленного устройства (в данном случае это HoloLens).</span><span class="sxs-lookup"><span data-stu-id="b5e5e-450">You will be prompted to insert the **IP address** of a remote device (the HoloLens, in this case, which you noted).</span></span>

    ![](images/AzureLabs-Lab302b-34.png)

5. <span data-ttu-id="b5e5e-451">Перейдите в меню **Сборка** и щелкните **Развернуть решение** , чтобы загружать неопубликованные приложение в HoloLens.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-451">Go to **Build** menu and click on **Deploy Solution** to sideload the application to your HoloLens.</span></span>

6. <span data-ttu-id="b5e5e-452">Теперь приложение должно отобразиться в списке установленных приложений в HoloLens, готовом к запуску.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-452">Your app should now appear in the list of installed apps on your HoloLens, ready to be launched!</span></span>

> [!NOTE]
> <span data-ttu-id="b5e5e-453">Чтобы выполнить развертывание в иммерсивное виртуальную гарнитуру, задайте для **платформы решения** значение *локальный компьютер*и задайте для параметра **Конфигурация** значение *Отладка*( *x86* в качестве **платформы**).</span><span class="sxs-lookup"><span data-stu-id="b5e5e-453">To deploy to immersive headset, set the **Solution Platform** to *Local Machine*, and set the **Configuration** to *Debug*, with *x86* as the **Platform**.</span></span> <span data-ttu-id="b5e5e-454">Затем выполните развертывание на локальном компьютере с помощью пункта меню " **Сборка** " и выберите *Развернуть решение*.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-454">Then deploy to the local machine, using the **Build** menu item, selecting *Deploy Solution*.</span></span> 

## <a name="to-use-the-application"></a><span data-ttu-id="b5e5e-455">Чтобы использовать приложение, выполните следующие действия.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-455">To use the application:</span></span>

<span data-ttu-id="b5e5e-456">Чтобы переключить функциональные возможности приложения между режимом *обучения* и режимом *прогнозирования* , необходимо обновить переменную **аппмоде** , расположенную в методе спящего режима **()** , расположенном в классе *имажекаптуре* .</span><span class="sxs-lookup"><span data-stu-id="b5e5e-456">To switch the app functionality between *Training* mode and *Prediction* mode you need to update the **AppMode** variable, located in the **Awake()** method located within the *ImageCapture* class.</span></span>

```
        // Change this flag to switch between Analysis mode and Training mode 
        AppMode = AppModes.Training;
```
<span data-ttu-id="b5e5e-457">или диспетчер конфигурации служб</span><span class="sxs-lookup"><span data-stu-id="b5e5e-457">or</span></span>
```
        // Change this flag to switch between Analysis mode and Training mode 
        AppMode = AppModes.Analysis;
```

<span data-ttu-id="b5e5e-458">В режиме *обучения* :</span><span class="sxs-lookup"><span data-stu-id="b5e5e-458">In *Training* mode:</span></span>

- <span data-ttu-id="b5e5e-459">Взгляните на **мышь** или **клавиатуру** и используйте **жест касания**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-459">Look at **Mouse** or **Keyboard** and use the **Tap gesture**.</span></span>

- <span data-ttu-id="b5e5e-460">Затем появится текст с запросом на указание тега.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-460">Next, text will appear asking you to provide a tag.</span></span>

- <span data-ttu-id="b5e5e-461">Например, с помощью **мыши** или **клавиатуры**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-461">Say either **Mouse** or **Keyboard**.</span></span>


<span data-ttu-id="b5e5e-462">В режиме прогнозирования:</span><span class="sxs-lookup"><span data-stu-id="b5e5e-462">In *Prediction* mode:</span></span>

- <span data-ttu-id="b5e5e-463">Взгляните на объект и используйте **жест касания**.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-463">Look at an object and use the **Tap gesture**.</span></span>

- <span data-ttu-id="b5e5e-464">Появится текст, предоставляющий обнаруженный объект, с наибольшей вероятностью (это нормализованное значение).</span><span class="sxs-lookup"><span data-stu-id="b5e5e-464">Text will appear providing the object detected, with the highest probability (this is normalized).</span></span>

## <a name="chapter-14---evaluate-and-improve-your-custom-vision-model"></a><span data-ttu-id="b5e5e-465">Глава 14. Оценка и усовершенствование модели Пользовательское визуальное распознавание</span><span class="sxs-lookup"><span data-stu-id="b5e5e-465">Chapter 14 - Evaluate and improve your Custom Vision model</span></span>

<span data-ttu-id="b5e5e-466">Чтобы сделать службу более точной, необходимо продолжить обучение модели, используемой для прогнозирования.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-466">To make your Service more accurate, you will need to continue to train the model used for prediction.</span></span> <span data-ttu-id="b5e5e-467">Это достигается благодаря использованию нового приложения, в котором используются оба режима *обучения* и *прогнозирования* , при этом требуется посетить портал, что рассматривается в этой главе.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-467">This is accomplished through using your new application, with both the *training* and *prediction* modes, with the latter requiring you to visit the portal, which is what is covered in this Chapter.</span></span> <span data-ttu-id="b5e5e-468">Будьте готовы к повторному посещению портала, чтобы постоянно улучшать модель.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-468">Be prepared to revisit your portal many times, to continually improve your model.</span></span>

1. <span data-ttu-id="b5e5e-469">Снова перейдите на портал Azure Пользовательское визуальное распознавание и в проекте выберите вкладку прогнозы (в верхнем центре страницы ):</span><span class="sxs-lookup"><span data-stu-id="b5e5e-469">Head to your Azure Custom Vision Portal again, and once you are in your project, select the *Predictions* tab (from the top center of the page):</span></span>

    ![](images/AzureLabs-Lab302b-35.png)

2. <span data-ttu-id="b5e5e-470">Вы увидите все образы, которые были отправлены в вашу службу во время работы приложения.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-470">You will see all the images which were sent to your Service whilst your application was running.</span></span> <span data-ttu-id="b5e5e-471">При наведении указателя мыши на изображения они будут предоставлять прогнозы, сделанные для этого образа:</span><span class="sxs-lookup"><span data-stu-id="b5e5e-471">If you hover over the images, they will provide you with the predictions that were made for that image:</span></span>

    ![](images/AzureLabs-Lab302b-36.png)

3. <span data-ttu-id="b5e5e-472">Выберите один из образов, чтобы открыть его.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-472">Select one of your images to open it.</span></span> <span data-ttu-id="b5e5e-473">После открытия вы увидите прогнозы, сделанные для этого изображения справа.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-473">Once open, you will see the predictions made for that image to the right.</span></span> <span data-ttu-id="b5e5e-474">Если прогнозы были правильными и вы хотите добавить этот образ в модель обучения службы, щелкните поле ввода " *Мои теги* " и выберите тег, который нужно связать.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-474">If you the predictions were correct, and you wish to add this image to your Service's training model, click the *My Tags* input box, and select the tag you wish to associate.</span></span> <span data-ttu-id="b5e5e-475">По завершении нажмите кнопку *сохранить и закрыть* в правом нижнем углу и продолжайте переходить к следующему изображению.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-475">When you are finished, click the *Save and close* button to the bottom right, and continue on to the next image.</span></span>

    ![](images/AzureLabs-Lab302b-37.png)

4. <span data-ttu-id="b5e5e-476">Когда вы вернетесь к сетке изображений, вы заметите, что изображения, в которые были добавлены теги (и сохранены), будут удалены.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-476">Once you are back to the grid of images, you will notice the images which you have added tags to (and saved), will be removed.</span></span> <span data-ttu-id="b5e5e-477">Если вы нашли какие-либо изображения, на которых нет элемента с тегами, их можно удалить, щелкнув такт на этом изображении (это можно сделать для нескольких изображений), а затем нажать кнопку *Удалить* в правом верхнем углу страницы сетки.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-477">If you find any images which you think do not have your tagged item within them, you can delete them, by clicking the tick on that image (can do this for several images) and then clicking *Delete* in the upper right corner of the grid page.</span></span> <span data-ttu-id="b5e5e-478">В следующем всплывающем окне можно щелкнуть *Да, удалить* или *нет*, чтобы подтвердить удаление или отменить его соответственно.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-478">On the popup that follows, you can click either *Yes, delete* or *No*, to confirm the deletion or cancel it, respectively.</span></span> 

    ![](images/AzureLabs-Lab302b-38.png)

5. <span data-ttu-id="b5e5e-479">Когда вы будете готовы к продолжению, нажмите зеленую кнопку « *обучение* » в правом верхнем углу.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-479">When you are ready to proceed, click the green *Train* button in the top right.</span></span> <span data-ttu-id="b5e5e-480">Модель службы будет обучена всеми предоставленными вами образами (что сделает его более точным).</span><span class="sxs-lookup"><span data-stu-id="b5e5e-480">Your Service model will be trained with all the images which you have now provided (which will make it more accurate).</span></span> <span data-ttu-id="b5e5e-481">После завершения обучения обязательно нажмите кнопку *сделать по умолчанию* , чтобы ваш *прогнозирующий URL-адрес* продолжал использовать самую последнюю итерацию службы.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-481">Once the training is complete, make sure to click the *Make default* button once more, so that your *Prediction URL* continues to use the most up-to-date iteration of your Service.</span></span>

    <span data-ttu-id="b5e5e-482">![](images/AzureLabs-Lab302b-39.png) ![](images/AzureLabs-Lab302b-40.png)</span><span class="sxs-lookup"><span data-stu-id="b5e5e-482">![](images/AzureLabs-Lab302b-39.png) ![](images/AzureLabs-Lab302b-40.png)</span></span>

## <a name="your-finished-custom-vision-api-application"></a><span data-ttu-id="b5e5e-483">Законченное приложение API Пользовательское визуальное распознавание</span><span class="sxs-lookup"><span data-stu-id="b5e5e-483">Your finished Custom Vision API application</span></span>

<span data-ttu-id="b5e5e-484">Поздравляем! вы создали приложение для смешанной реальности, которое использует API Azure Пользовательское визуальное распознавание для распознавания реальных объектов, обучения модели службы и получения уверенности в том, что появлялось.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-484">Congratulations, you built a mixed reality app that leverages the Azure Custom Vision API to recognize real world objects, train the Service model, and display confidence of what has been seen.</span></span>

![](images/AzureLabs-Lab302b-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="b5e5e-485">Премиальные упражнения</span><span class="sxs-lookup"><span data-stu-id="b5e5e-485">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="b5e5e-486">Упражнение 1</span><span class="sxs-lookup"><span data-stu-id="b5e5e-486">Exercise 1</span></span>

<span data-ttu-id="b5e5e-487">Обучить **Пользовательская служба визуального распознавания** для распознавания большего числа объектов.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-487">Train your **Custom Vision Service** to recognize more objects.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="b5e5e-488">Упражнение 2</span><span class="sxs-lookup"><span data-stu-id="b5e5e-488">Exercise 2</span></span>

<span data-ttu-id="b5e5e-489">Чтобы расширить свои знания, выполните следующие упражнения.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-489">As a way to expand on what you have learned, complete the following exercises:</span></span>

<span data-ttu-id="b5e5e-490">Подавать звуковой сигнал при распознавании объекта.</span><span class="sxs-lookup"><span data-stu-id="b5e5e-490">Play a sound when an object is recognized.</span></span>

### <a name="exercise-3"></a><span data-ttu-id="b5e5e-491">Упражнение 3</span><span class="sxs-lookup"><span data-stu-id="b5e5e-491">Exercise 3</span></span>

<span data-ttu-id="b5e5e-492">Используйте API для переобучения службы с теми же изображениями, которые анализирует приложение, чтобы сделать службу более точной (одновременное прогнозирование и обучение).</span><span class="sxs-lookup"><span data-stu-id="b5e5e-492">Use the API to re-train your Service with the same images your app is analyzing, so to make the Service more accurate (do both prediction and training simultaneously).</span></span>
