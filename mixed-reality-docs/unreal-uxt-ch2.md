...
В этом руководстве мы начнем работу с создания приложения Unreal для HoloLens 2. Нам предстоит добавить подключаемый модуль HoloLens, создать уровень, задать его освещение, а также добавить на него шахматную доску и фигуру. Для этого мы воспользуемся готовыми ресурсами для трехмерной шахматной фигуры и материалов, так что можно не беспокоиться — создавать модели с нуля не придется. Когда вы выполните все описанное в руководстве, у вас будет пустой холст, готовый для размещения объектов смешанной реальности.

Прежде чем продолжать, убедитесь, что у вас есть все необходимое из списка на странице [Начало работы](https://docs.microsoft.com/windows/mixed-reality/unreal-uxt-ch1).
...
Прежде чем добавлять объекты в сцену, необходимо включить два подключаемых модуля.
...
Теперь у вас есть пустой уровень, который можно чем-нибудь заполнить.

## <a name="creating-a-level"></a>Создание уровня
Следующая задача — создать простую игровую обстановку с начальной точкой и кубом, задающим начало отсчета и масштаб.
...
2. На вкладке **Modes** (Режимы) выберите **Basic** (Основные) и перетащите в сцену элемент **PlayerStart**. 
    * На вкладке **Details** (Сведения) установите для параметра **Location** (Расположение) значения **X = 0**, **Y = 0** и **Z = 0**, чтобы после запуска приложения пользователь начинал игру в центре сцены.
...
5. Выберите **File > Save Current** (Файл > Сохранить текущий), присвойте уровню имя **Main** (Главный) и нажмите кнопку **Save** (Сохранить). 
...
В настроенную сцену можно теперь добавить шахматную доску и фигуру, чтобы получилась законченная пространственная среда приложения.
...
1. Скачайте с [GitHub](https://github.com/microsoft/MixedReality-Unreal-Samples/blob/master/ChessApp/ChessAssets.7z) и распакуйте папку с ресурсами.

2. В обозревателе содержимого (**Content Browser**) выберите **Add New > New Folder** (Добавить > Папка) и присвойте новой папке имя **ChessAssets**. 
    * Дважды щелкните созданную только что папку — в нее мы будем импортировать трехмерные ресурсы.
...
3. В обозревателе содержимого (**Content Browser**) нажмите кнопку **Import** (Импорт), выберите все элементы из папки с распакованными ресурсами и нажмите кнопку **Open** (Открыть). 
    * Папка содержит файлы трехмерных сеток шахматной доски и фигур в формате FBX, а также файлы текстурных карт в формате TGA, на основе которых мы создадим материалы для доски и фигур.  
...
1. В обозревателе содержимого (**Content Browser**) выберите **Add New > New Folder** (Добавить > Папка) и присвойте новой папке имя **Content Browser**. 
...
2. На панели **Components** (Компоненты) выберите **Add Component > Scene** (Добавить компонент > Сцена) и присвойте новой сцене имя **Root**. Обратите внимание, что **Root** отобразится как дочерний объект элемента **DefaultSceneRoot** (см. снимок экрана ниже).
...
4. На панели **Components** (Компоненты) выберите **Add Component > Static Mesh** (Добавить компонент > Статическая сетка) и присвойте новой сетке имя **SM_Board**. Она отобразится как дочерний объект элемента **Root**.

![Добавление статической сетки](images/unreal-uxt/2-sm-board.PNG)

4. Щелкните элемент **SM_Board**, прокрутите панель **Details** (Сведения) до раздела **Static Mesh** (Статическая сетка) и выберите в раскрывающемся списке вариант **ChessBoard**. 

![Сетка шахматной доски в окне просмотра](images/unreal-uxt/2-sm-board-view.PNG)

5.  В той же панели **Details** (Сведения) разверните раздел **Materials** (Материалы) и выберите в раскрывающемся списке **Create New Asset > Material** (Создать ресурс > Материал). 
...
3.  Нажмите кнопку **Play** (Воспроизведение), чтобы просмотреть созданную доску на уровне. Нажмите клавишу **ESC**, чтобы вернуться в редактор. 

Теперь по тому же алгоритму, что и с доской, создадим шахматную фигуру:

1. Дважды щелкните папку **Blueprints**, щелкните правой кнопкой мыши и выберите **Blueprint Class** (Класс схемы), а затем **Actor** (Субъект). Присвойте субъекту имя **WhiteKing**.

2. Дважды щелкните элемент **WhiteKing**, чтобы открыть его в редакторе схем, выберите **Add Component > Scene** (Добавить компонент > Сцена) и присвойте сцене имя **Root**. 
...
Готово! Нажмите кнопку **Play** (Воспроизведение), чтобы увидеть заполненный уровень в действии, а когда захотите выйти, нажмите клавишу **ESC**. В этом руководстве подробно описано создание простого проекта. Следующая часть серии посвящена настройке проекта для смешанной реальности. 
