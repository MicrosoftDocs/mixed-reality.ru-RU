...
Мы хотели воспользоваться всеми преимуществами использования HoloLens для отрисовки трехмерных объектов непосредственно в своем собственном пространстве, поэтому мы решили создать реалистичный взгляд на Galaxy, где люди смогут увеличить и увидеть отдельные звезды, каждый на своем траекторий .

На первой неделе разработки мы поступили несколько целей для нашего представления Млечный путьного способа Galaxy: Необходимо иметь глубину, движение и объемные — полные звезды, которые помогут создать форму Galaxy.
...
Один из наших участников команды уже написал C# код, который мог бы создать звезды на их начальной позиции. Звезды находятся на эллипсе, а их положения могут быть описаны (**курвеоффсет**, **еллипсесизе**, **повышение прав**), где **курвеоффсет** — угол звезды на эллипсе, **еллипсесизе** — это размер эллипса. вдоль X и Z, а также повышения прав на правильное повышение уровня звездочки в Galaxy. Таким образом, можно создать буфер ([Компутебуффер Unity](http://docs.unity3d.com/ScriptReference/ComputeBuffer.html)), который будет инициализирован с каждым атрибутом звезды, и отправить его на GPU, где он будет использоваться для оставшейся работы. Чтобы нарисовать этот буфер, мы используем [Дравпроцедурал Unity](http://docs.unity3d.com/ScriptReference/Graphics.DrawProcedural.html) , который позволяет запускать шейдер (код на GPU) в произвольном наборе точек без фактической сетки, представляющей Galaxy:
...
Наша группа выполнила некоторые исследования о способе галаксиесной функции, и мы сделали пользовательскую систему частиц специально для Galaxy, чтобы мы могли передвигать частицы на эллипсы на основе «[теории](https://en.wikipedia.org/wiki/Density_wave_theory)» плотности, которые сеоризес, что руки Galaxy являются областями более высокая плотность, но в постоянных Flux, например Застревание трафика. Он выглядит стабильным и сплошным, но звезды фактически перемещаются и выходят с руки по мере их перемещения по соответствующим эллипсам. В нашей системе частицы никогда не существовали на ЦП — мы создаем карты и расходим их всем на GPU, так что вся система – это просто начальное состояние и время. Он выполняется следующим образом:
