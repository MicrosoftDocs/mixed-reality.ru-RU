...
>[!NOTE] Избегайте GetComponent(string) <br/>
...
Легко приближенное значение для сравнения шейдеров в производительности — чтобы определить среднее число операций каждого выполняется во время выполнения. Это можно сделать довольно легко в Unity.
