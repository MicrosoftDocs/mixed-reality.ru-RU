...
>[!NOTE]
> Update () является наиболее распространенной частью этой проблемы с производительностью, но другие повторяющиеся обратные вызовы Unity, такие как следующие, могут быть так же неплохы, если не хуже: Фикседупдате (), Латеупдате (), Онпострендер ", OnPreRender (), Онрендеримаже () и т. д. 
...
Кроме того, рекомендуется отключить все перекрытие теневых копий, так как это также приводит к добавлению затрат на GPU в сцене Unity. Тени можно отключить для отдельных источников, но можно также управлять глобально с помощью параметров качества.
