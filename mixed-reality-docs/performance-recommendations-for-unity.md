...
Также настоятельно рекомендуется, чтобы разработчики Просмотрели [Рекомендуемые параметры среды для статьи Unity](Recommended-settings-for-unity.md). В этой статье содержится содержимое с некоторыми из наиболее важных конфигураций сцены для создания производительных приложений смешанной реальности. Некоторые из этих рекомендуемых параметров также выделены ниже.
...
Рекомендуется кэшировать ссылки на все соответствующие компоненты и объекты gameobject при инициализации. Это обусловлено тем, что повторяющиеся вызовы функций, такие как *[\<t > ()](https://docs.unity3d.com/ScriptReference/GameObject.GetComponent.html)* , значительно дороже, чем затраты на память для хранения указателя. Это также относится к очень регулярно используемой [камере. Main](https://docs.unity3d.com/ScriptReference/Camera-main.html). В действительности *камера. Main* просто использует *[финдгамеобжектсвистаг ()](https://docs.unity3d.com/ScriptReference/GameObject.FindGameObjectsWithTag.html)* под, что дорогостоящий Поиск объекта Camera в графе сцены с помощью тега *"маинкамера"* .
...
3) **Будьте осторожны при упаковке**
    ...
    Одной из наиболее распространенных форм упаковки-преобразования в разработке является использование [типов значений, допускающих значение NULL](https://docs.microsoft.com//dotnet/csharp/programming-guide/nullable-types/). Часто требуется, чтобы функция возвращала значение NULL для типа значения в функции, особенно в том случае, если операция может не пытаться получить значение. Потенциальная проблема при таком подходе заключается в том, что распределение выполняется в куче и, следовательно, должно быть собрано позже.

    **Пример упаковки-преобразования вC#**
    ...
    **Пример проблемной упаковки с помощью типов значений, допускающих значения NULL**

    Этот код демонстрирует фиктивный класс частиц, который может быть создан в проекте Unity. Вызов `TryGetSpeed()` приведет к выделению объекта в куче, который должен быть собран сборщиком мусора в более позднем момент времени. Этот пример особенно проблематичен, так как в сцене может быть больше 1000 или много других частиц, каждая из которых запрашивает текущую скорость. Таким образом, было бы выделено 1000 объектов, что, следовательно, освобождается каждый кадр, что значительно снизит производительность. Повторное написание функции для возврата отрицательного значения, например-1, чтобы указать, что ошибка помешала бы избежать этой проблемы и удерживать память в стеке.
...
>[!NOTE]
> Update () является наиболее распространенной частью этой проблемы с производительностью, но другие повторяющиеся обратные вызовы Unity, такие как, могут быть так же неплохы, если не хуже: Фикседупдате (), Латеупдате (), Онпострендер ", OnPreRender (), Онрендеримаже () и т. д. 
...
3) **Избегайте использования интерфейсов и виртуальных конструкций**
    ...
    Как правило, рекомендация заключается в том, чтобы не помечать поля и функции как виртуальные, если нет четкого предположения о том, что этот член не требуется перезаписывать. Одна из них должна быть особенно осторожными в путях кода с высокой частотой, которые вызываются несколько раз для каждого кадра или даже один раз для каждого кадра, например метода `UpdateUI()`.

4) **Избегайте передачи структур по значению**

    В отличие от классов, структуры являются типами значений и при передаче непосредственно в функцию их содержимое копируется в только что созданный экземпляр. Эта копия добавляет стоимость ЦП, а также дополнительную память в стеке. Для небольших структур такой результат обычно очень минимальный, а значит, приемлем. Однако, если возможно, для функций повторно вызывается каждый кадр, а также функции, принимающие крупные структуры, по возможности измените определение функции для передачи по ссылке. Дополнительные сведения см. [здесь](https://docs.microsoft.com/dotnet/csharp/programming-guide/classes-and-structs/how-to-know-the-difference-passing-a-struct-and-passing-a-class-to-a-method).

#### <a name="miscellaneous"></a>Прочее

1) **Физики**

    ответ. как правило, самый простой способ улучшить физику — ограничить количество времени, затраченного на физическое или количество итераций в секунду. Разумеется, это снизит точность имитации. См. раздел [тимеманажер](https://docs.unity3d.com/Manual/class-TimeManager.html) в Unity.

    б) тип конфликтующих данных в Unity имеет широко разные характеристики производительности. В приведенном ниже порядке перечислены наиболее производительные конфликтующие по крайней мере более производительные конфликтующие. Наиболее важно избегать использования противоречащих друг другу, которые значительно дороже, чем простые конфликты.
...
1)  Откройте **Параметры проигрывателя XR** (перейдите в раздел **Правка** > **параметры проекта** > параметры **проигрывателя** > **XR**).
...
Более того, обычно предпочтительнее объединять сетки в один GameObject, где это возможно и разумно. Каждый модуль подготовки в Unity будет иметь связанные с ними вызовы рисования и отправлять объединенную сетку в одном модуле подготовки отчетов.
...
Обычно рекомендуется включить **совместное использование буфера глубины** в **параметрах XR проигрывателя** , чтобы оптимизировать [стабильность](Hologram-stability.md). При включении повторного проецирования на основе глубины с помощью этого параметра рекомендуется выбрать **16-разрядный формат глубины** , а не **24-разрядный формат глубины**. 16-разрядные буферы глубины радикально снижают пропускную способность (и, таким образом, энергопотребление), связанную с трафиком в буфере глубины. Это может быть значительным выигрышем в снижении энергопотребления и улучшению производительности. Однако существует два возможных отрицательных результата с использованием *16-разрядного формата глубины*.

**Борьба с Z**

Уменьшенная точность диапазона глубины упрощает отмедленную отмедление [z](https://en.wikipedia.org/wiki/Z-fighting) с 16-разрядными, чем 24-разрядными. Чтобы избежать этих артефактов, измените близкие и далеко вырезанные плоскости [камеры Unity](https://docs.unity3d.com/Manual/class-Camera.html) , чтобы учитывать меньшую точность. Для приложений на основе HoloLens далеко плоскость 50 млн вместо 1000m по умолчанию Unity может, как правило, устранить любые отрезки от z.

**Буфер отключенного шаблона**

Когда Unity создает [текстуру рендеринга с 16-разрядной глубиной](https://docs.unity3d.com/ScriptReference/RenderTexture-depth.html), буфер трафарета не создается. При выборе 24-разрядного формата глубины для каждой документации Unity создается 24-битовый z-буфер, а также [8-разрядный буфер трафаретов] (https://docs.unity3d.com/Manual/SL-Stencil.html) (если на устройстве применимо значение 32-bit, которое, как правило, является HoloLens).

### <a name="avoid-full-screen-effects"></a>Избегайте полноэкранных эффектов

Методы, работающие в полноэкранном режиме, могут быть достаточно ресурсоемкими, так как их порядок равен миллионам операций в каждом кадре. Поэтому рекомендуется избегать [действий, выполняемых после обработки](https://docs.unity3d.com/Manual/PostProcessingOverview.html) , таких как сглаживание, раскрытия и т. д.

### <a name="optimal-lighting-settings"></a>Параметры оптимального освещения

[Глобальные освещения в режиме реального времени](https://docs.unity3d.com/Manual/GIIntro.html) в Unity могут предоставлять непревзойденные визуальные результаты, но при этом выполняются довольно ресурсоемкие вычисления освещения. Рекомендуется отключить глобальное освещения в режиме реального времени для каждого файла сцены Unity с помощью **окна** > **подготовки** > **параметров освещения** > снять флажок **глобальные освещения в реальном времени**.

Более того, рекомендуется отключить все перекрытие теневых копий, так как это также приводит к добавлению дорогостоящих проходов GPU в сцену Unity. Тени можно отключить для отдельных источников, но можно также управлять глобально с помощью параметров качества.

**Измените** > **Параметры проекта**, а затем выберите категорию **качества** > выберите **низкое качество** для платформы UWP. Можно также просто установить свойство **Shadows** для **отключения теней**.
...
1) Выберите ресурс шейдера или материал, а затем в правом верхнем углу окна инспектора щелкните значок шестеренки, а затем **выберите "выбрать шейдер"** .
...
#### <a name="optimize-pixel-shaders"></a>Оптимизация шейдеров пикселей

Просмотрев результаты скомпилированных статистических результатов с помощью приведенного выше метода, [шейдер фрагментов](https://en.wikipedia.org/wiki/Shader#Pixel_shaders) обычно выполняет больше операций, чем [шейдер вершин](https://en.wikipedia.org/wiki/Shader#Vertex_shaders), в среднем. Шейдер фрагментов, также называемый шейдером пикселей, выполняется на экране в выводе экрана, а шейдер вершин выполняется только на вершину всех сеток, отображаемых на экране. 
...
Вместо использования физической отрисовки (PBR) или другого высококачественного шейдера ознакомьтесь с использованием более производительного и более дешевого шейдера. [Набор средств Mixed Reality](https://github.com/Microsoft/MixedRealityToolkit-Unity) предоставляет [стандартный шейдер мртк](https://microsoft.github.io/MixedRealityToolkit-Unity/Documentation/README_MRTKStandardShader.html) , оптимизированный для проектов смешанной реальности.
...
Используйте *предварительную загрузку шейдера* и другие приемы для оптимизации [времени загрузки шейдера](https://docs.unity3d.com/Manual/OptimizingShaderLoadTime.html). В частности, Предварительная загрузка шейдера означает, что вы не увидите хитчес из-за компиляции шейдера времени выполнения.

### <a name="limit-overdraw"></a>Ограничить перерисовку

В Unity одна может отображать перерисовку для своей сцены, переключаясь в [**меню режима рисования**](https://docs.unity3d.com/Manual/ViewModes.html) в левом верхнем углу **представления сцены** и выбрав пункт **перерисовка**.
...
Чрезмерное выделение памяти & операциях изъятия может оказать негативное воздействие на приложение holographic, что приведет к нестабильной производительности, замороженным кадрам и другим последствиям неблагоприятного воздействия. Особенно важно понимать требования к памяти при разработке в Unity, так как управление памятью управляется сборщиком мусора.

#### <a name="garbage-collection"></a>Сборка мусора

Holographic приложений потеряет время вычислений в сборщике мусора (GC) при активации GC для анализа объектов, которые больше не находятся в области во время выполнения, и нужно освободить память, чтобы сделать их доступными для повторного использования. Для постоянных распределений и отмены распределения обычно требуется, чтобы сборщик мусора выполнялся чаще, что приводит к ухудшению производительности и удобства работы пользователей.
...
- Удалите вызовы Debug. log (), если они больше не нужны, так как они все еще выполняются во всех версиях сборки приложения.
...
Рекомендуется запустить приложение с небольшой сценой, а затем использовать *[сценеманажер. лоадсценеасинк](https://docs.unity3d.com/ScriptReference/SceneManagement.SceneManager.LoadSceneAsync.html)* для загрузки оставшейся части сцены. Это позволяет приложению перейти в интерактивное состояние как можно быстрее. Имейте в виду, что во время активации новой сцены может возникнуть большой Пиковый объем ресурсов ЦП, а все отображаемое содержимое может перебои или Хитч. Один из способов обойти это — установить для свойства AsyncOperation. Алловсценеактиватион значение false в загружаемой сцене, подождать загрузки сцены, очистить экран до черного, а затем снова установить значение true, чтобы завершить активацию сцены.

Помните, что во время загрузки сцены запуска будет отображаться экран-заставка Holographic.
