...
# <a name="mr-basics-101-complete-project-with-device"></a>MR Basics 101: Полный проект с устройством

<br>

>[!VIDEO https://www.youtube.com/embed/XKIIEC5BMWg]

Этот учебник поможет в проекте, встроенные в Unity, который демонстрирует основные возможности Windows Mixed Reality, в том числе HoloLens [помощи](gaze.md), [жесты](gestures.md), [голоса](voice-input.md), [пространственных звук](spatial-sound.md) и [пространственное сопоставление](spatial-mapping.md).

Руководства займет примерно 1 час.
...
<td>MR Basics 101: Полный проект с устройством</td><td style="text-align: center;"> ✔️</td><td style="text-align: center;"> </td>
</tr>
</table>
...
* Так как **Origami** проект не содержит сцены, сохраните сцену пустым по умолчанию в новый файл с помощью: **Файл** / **сохранить сцену как**.
* Назовите новую сцену **Origami** и нажмите клавишу **Сохранить** кнопки.

#### <a name="setup-the-main-virtual-camera"></a>Настройка главной виртуальной камеры
...
* Из **голограммы** папку «проект» (развернуть ресурсы и выберите голограммы или дважды щелкните папку голограммы «проект»):
...
* Открыть (двойной щелчок) **Origami.sln**.
* С помощью верхней панели инструментов в Visual Studio, изменить целевой объект с отладки на **выпуска** и из ARM для **X86**.
* Щелкните стрелку рядом с кнопкой на устройстве и выберите **удаленный компьютер** для развертывания по Wi-Fi.
  ...
  * HoloLens, подключенного через USB, можно вместо этого выбрать **устройства** для развертывания по USB.
  * Оставьте **режим проверки подлинности** присвоено **универсальной**.
  * Нажмите кнопку **выберите**

* Нажмите кнопку **Отладка > Запуск без отладки** или нажмите клавишу **Ctrl + F5**. Если это при первом развертывании к устройству, необходимо будет [свяжите его с помощью Visual Studio](using-visual-studio.md#pairing-your-device-hololens-(1st-gen)).

* Проект Origami будет теперь сборки, развертывания вашей HoloLens и запустите.
* Поместите на вашей HoloLens и оглядитесь для вашего нового голограммы см. в разделе.
...
* Вернитесь к ранее используется для развертывания вашего HoloLens решения Visual Studio.
* Выберите «Обновить все» при появлении запроса.
* Нажмите кнопку **Отладка -> Запуск без отладки** или нажмите клавишу **Ctrl + F5**.
* Теперь оглядитесь сцены и обратите внимание на то, как курсор взаимодействует с форму объектов.
...
Мы начнем с создания сценария, то может обнаружить выберите жест.
...
* Экспорт, построение и развертывание приложения в вашей HoloLens.
* Рассмотрим один из сферы.
* Сделайте жест select и просмотр сферы фигуру на область, расположенную ниже.

## <a name="chapter-4---voice"></a>Глава 4 – голоса

>[!VIDEO https://www.youtube.com/embed/1-Aq0VVtHM8]

В этой главе мы добавим поддержку для двух [голосовые команды](voice-input.md): «Сброс world» вернуть удаленные как шары, расположенные в их исходное расположение, перетаскивания «сферы» чтобы делятся сферы.
...
* Экспорт, построение и развертывание приложения в вашей HoloLens.
* Посмотреть на один из сферы и сказать «**Drop Sphere**«.
...
* В правой части панели Инспектора найти **подключаемый модуль Spatializer** задание и выберите **MS HRTF Spatializer**.
* Из **голограммы** папку «проект», перетащите **окружением** объекта на **OrigamiCollection** объект на панели «иерархии».
* Выберите **OrigamiCollection** и найти **источника аудио** компонент на панели инспектора. Измените следующие свойства:
  * Проверьте **Spatialize** свойство.
  * Проверьте **играть на переходит в спящий режим**.
  * Изменение **пространственных Blend** для **3D** путем перетаскивания бегунка вплоть до справа. Значение должно изменяться от 0 до 1, при перемещении ползунка.
...
* Перетаскивание **SphereSounds** для **Sphere1** и **Sphere2** объектов в иерархии.
...
* Экспорт, построение и развертывание приложения в вашей HoloLens.
* Переместите ближе и максимально далеко от рабочей области и включить для параллельных звуковое изменить.
...
* В Unity, щелкните **голограммы** папку «проект».
...
* В **панели Инспектора**, измените следующие свойства:
  ...
  * Найдите **рисования материал** и щелкните этот кружок в правой части. Тип "**каркас**" в поле поиска вверху. Щелкните результат, а затем закройте окно. При этом для рисования материал должен получить значение каркасной модели.
* Экспорт, построение и развертывание приложения в вашей HoloLens.
* При запуске приложения, каркаса сетки отправляет сообщение в реальном мире.
...
* Теперь теперь можно поместить игры в определенном расположении путем gazing на него, с помощью Select жест перемещения в новое расположение и еще раз с помощью Select жест.

## <a name="chapter-7---holographic-fun"></a>Глава 7 - Holographic fun

### <a name="objectives"></a>Цели

* Показать входом holographic underworld.

### <a name="instructions"></a>Инструкция

Теперь мы покажем, как выявлять holographic underworld:

* Из **голограммы** папку «проект»:
  * Перетащите **Underworld** в иерархию, чтобы быть дочерним элементом **OrigamiCollection**.
* В **сценарии** папки, создайте сценарий с именем **HitTarget**.
* В **иерархии**, разверните **OrigamiCollection**.
* Разверните **этап** объекта и выберите **целевой** объекта (синий вентилятора).
* Перетащите **HitTarget** скрипт на **целевой** объекта.
* Откройте **HitTarget** сценариев в Visual Studio и обновить его следующим:
...
* В Unity, выберите **целевой** объекта.
* Два открытых свойства теперь отображаются на **попаданий целевой** компонента и необходимости ссылаться на объекты в сцене наших:
  * Перетащите **Underworld** из **иерархии** панели **Underworld** свойство **попаданий целевой** компонента.
  * Перетащите **этап** из **иерархии** панели **объекта, чтобы скрыть** свойство **попаданий целевой** компонента.
* Экспорт, построения и развертывания приложения.
* Размещение коллекции Origami на ближайшее снизу целое и затем использовать жест Select для принятия сферы drop.
* Когда сферы достигает целевого (синий вентилятора), будет выполняться развертывание. Коллекции будут скрыты, и появится отверстия для underworld.
...
* Как сделать использование взглядом, жест, голоса, звук и пространственное сопоставление.
* Как создавать и развертывать приложения с помощью Visual Studio.

Теперь вы готовы приступить к созданию собственных голографический интерфейс!

## <a name="see-also"></a>См. также

* [Общие сведения об MR 101E: Полный проект с помощью эмулятора](holograms-101e.md)
* [Взглядом](gaze.md)
* [Жесты](gestures.md)
