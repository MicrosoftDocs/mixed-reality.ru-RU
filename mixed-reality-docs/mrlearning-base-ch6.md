...
* В поле **расположение для размещения** назначьте соответствующий объект плацеменсинтс, в данном случае — объект **RoverEnclosure_PlacementHints**
* В поле " **объект подсказки** " назначьте соответствующий тултипобжект (в данном случае это объект **RoverEnclosure_ToolTip**
...
* Сбросьте приложение таким образом, чтобы Лунный модуль и вся часть были возвращены обратно в исходное расположение.
...
* Назначьте каждый из **дочерних объектов** объекта **плацеменсинтс** полю **element** в массиве объектов Game:
...
Если теперь вы перейдете в режим игры и нажмете кнопку Reset (Сброс), вы услышите воспроизводимый звуковой клип и увидите, как запустился Лунный модуль:
...
В окне Иерархия выберите кнопки Роккетлаунчер > > объект **хинтсбуттон** , затем в компоненте, **поддерживающем нажатие кнопки (скрипт)** , создайте новое событие **нажатия кнопки ()** , настройте объект **лунармодуле** для получения события и определите **тогглеплацеменсинтс. тогглегамеобжектс** действие, которое необходимо запустить:
