...
Наконец, обновите логику ввода, чтобы распознать изменения в состоянии контроллера. Здесь мы используем ту же переменную m_pointerPressed, которая обсуждалась в разделе выше для добавления событий мыши. Добавьте его в метод Update, непосредственно перед тем, как он проверяет наличие Спатиалпоинтерпосе:
