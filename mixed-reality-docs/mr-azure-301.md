...
# <a name="mr-and-azure-301-language-translation"></a>MR и Azure 301: систем перевода.
...
<td> MR и Azure 301: систем перевода.</td><td style="text-align: center;"> ✔️</td><td style="text-align: center;"> ✔️</td>
</tr>
</table>
...
10. На странице *быстрого запуска* службы *перевод текстов* перейдите к первому шагу, *Возьмите ключи*и щелкните **ключи** (это можно также сделать, щелкнув синие клавиши-гиперссылки, расположенные в меню навигации службы. отмечается значком ключа). При этом будут раскрыты *ключи*службы.
...
> [!NOTE]
> Для этого курса не потребуется контроллеры движения. Если вам нужна поддержка настройки иммерсивного головного телефона, выполните следующие [действия](https://support.microsoft.com/en-au/help/4043101/windows-10-set-up-windows-mixed-reality).
...
2.  Теперь необходимо указать имя проекта Unity. Вставьте **MR_Translation**. Убедитесь, что для типа проекта задано значение **3D**. Задайте для *расположения нужное расположение* (Помните, что ближе к корневым каталогам лучше). Затем нажмите кнопку **создать проект**.
...
5.  Перейдите в раздел **файл > параметры сборки** и убедитесь в том, что:
    ...
    6. Сохраните сцену и добавьте ее в сборку.
        ...
        3. Откройте созданную папку **сцены** , а затем в текстовом поле *имя файла*введите **MR_TranslationScene**, а затем нажмите кнопку **сохранить**.
...
11. Затем в окне новое, небольшое *цвета* измените *шестнадцатеричный цвет* на: **0032EAFF**
...
2.  Создав папку **Scripts** , дважды щелкните ее, чтобы открыть. Затем в этой папке щелкните правой кнопкой мыши и выберите **Создать >** затем  **C# сценарий**. Назовите *результаты*скрипта. 
...
2.  Щелкните правой кнопкой мыши в  папке Scripts и выберите **создать C# > скрипт**. Назовите сценарий *микрофонеманажер*. 
...
5.  Затем добавьте следующие переменные в класс *Translator* :
    ...
    > [!NOTE]
    > - Языки, вставленные в перечисление языков, являются просто примерами. Вы можете добавить дополнительные сведения, если хотите. [API поддерживает более 60 языков](https://docs.microsoft.com/azure/cognitive-services/translator/languages) (включая марсианского)!
    > - Существует [более интерактивная страница](https://www.microsoft.com/translator/business/languages/), посвященная доступным языкам, но имейте в виду, что страница работает только в том случае, если для языка сайта задано значение en-US (а веб-сайт Майкрософт, скорее всего, перенаправляется на ваш собственный язык). Язык сайта можно изменить в нижней части страницы или путем изменения URL-адреса.
...
- Микрофон работает при запуске сцены (если нет, убедитесь, что подключенный микрофон является устройством *по умолчанию* и что вы [правильно настроили его в Windows](https://support.microsoft.com/en-au/help/4027981/windows-how-to-set-up-and-test-microphones-in-windows-10)).
