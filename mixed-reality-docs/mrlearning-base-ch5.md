...
В компоненте обработчика речевого ввода (скрипт) нажмите кнопку мелкий **+** , чтобы добавить элемент keyword в список ключевых слов:

![mrlearning-base](images/mrlearning-base/tutorial5-section1-step5-1.png)

Щелкните созданный **элемент 0** , чтобы развернуть его, а затем в раскрывающемся списке **ключевых слов** выберите ключевое слово **воспроизвести музыку** , созданное ранее:
...
В окне Иерархия **Создайте четыре Куба** в качестве дочерних объектов объекта **панжестуре** и задайте для их **шкалы** преобразования значение X = 0,15, Y = 0,15, Z = 0,15:
...
В окне Иерархия выберите объект **Мак** , а затем создайте новое событие **on Selected ()** , настройте объект **Мак** для получения события и определите **эйетраккингтуториалдемо. блиптаржет** в качестве действия для запуска:
