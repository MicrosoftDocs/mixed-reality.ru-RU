...
Идея состоит в том, что голограммы или элементы пользовательского интерфейса реагируют на инстинктуале, что может даже не показаться, что пользователь взаимодействует с системой вообще, а сам система и пользователь синхронизированы. Одним из примеров является **Автоматическая прокрутка на основе взгляда** , когда пользователь считывает текст, когда текст прокручивается или помещается в синхронизацию с помощью взгляда пользователя. Ключевым аспектом этого процесса является то, что скорость прокрутки адаптируется к скорости чтения пользователя.
...
- **Выразительные аватары.** Отслеживание взгляда помогает в более выразительных трехмерных аватарах, используя данные отслеживания взгляда, чтобы анимировать глаза аватара, указывающие, что именно видят пользователи. Он также повышает выразительность за счет добавления мигающих. 
...
Чтобы отслеживание взгляда работало точно, каждому пользователю нужно пройти калибровку отслеживания взгляда. В HoloLens 2 пользователю предлагается откалибровать визуальные элементы во время настройки устройства, просмотрев набор с фиксациейных целей. Это позволяет устройству настроить удобное и качественное представление для пользователя и обеспечить точное отслеживание в то же время.  Для большинства пользователей калибровка должна работать, но бывают случаи, когда пользователь не может успешно выполнить калибровку.  Дополнительные сведения о калибровке см. в статье [калибровка](https://docs.microsoft.com/en-us/windows/mixed-reality/calibration).
...
* [Калибровка](https://docs.microsoft.com/en-us/windows/mixed-reality/calibration)
