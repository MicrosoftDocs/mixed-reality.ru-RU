...
## <a name="getting-the-windows-mixed-reality-openxr-developer-portal"></a>Получение портала разработчика Опенкср для Windows Mixed Reality

Чтобы испытать среду выполнения Опенкср в Windows Mixed Reality, можно установить <a href="https://www.microsoft.com/store/productId/9n5cvvl23qbt" target="_blank">приложение портала разработчика Mixed Reality опенкср</a>.  Это приложение содержит демонстрационную сцену, в которой реализованы различные функции Опенкср, а также страница состояния системы, которая содержит основные сведения об активной среде выполнения и текущей гарнитуре.

При использовании эмулятора самый простой способ установить портал разработчика для смешанной реальности Опенкср с помощью [портала устройств Windows](using-the-windows-device-portal.md), перейдя на страницу "опенкср" и нажав кнопку "установить" в разделе "компоненты разработчика". (это также работает на физическом устройстве HoloLens 2.)
...
Вы можете либо [ссылаться на официальный пакет NuGet опенкср](#reference-official-openxr-nuget-package) из проекта Visual Studio, либо [включить официальный источник опенкср Loader](#include-official-openxr-loader-source) из репозитория кхронос GitHub.  Любой из этих подходов предоставит вам доступ к основным функциям Опенкср 1,0, а также опубликованным `KHR`, `EXT` и `MSFT`ным расширениям.

Если вы хотите поэкспериментировать с `MSFT_preview`ными расширениями, вы можете [скопировать их в заголовках опенкср предварительной версии](#using-preview-extensions) из репозитория GitHub в смешанной реальности.

### <a name="reference-official-openxr-nuget-package"></a>Ссылка на официальный пакет NuGet Опенкср

<a href="https://www.nuget.org/packages/OpenXR.Loader/" target="_blank">Пакет NuGet **Опенкср. Loader** </a> — это самый простой способ ссылки на предварительно созданный загрузчик опенкср. DLL в решении Visual Studio C++ .  Это обеспечит доступ к функциям ядра Опенкср 1,0, а также опубликованным `KHR`, `EXT` и `MSFT`.
...
Значение, если необходимо самостоятельно создать загрузчик, например, чтобы избежать дополнительного загрузчика. DLL, вы можете извлечь официальные источники загрузчика Кхронос Опенкср в проект.  Это обеспечит доступ к функциям ядра Опенкср 1,0, а также опубликованным `KHR`, `EXT` и `MSFT`.
...
Расширения `MSFT_preview`, перечисленные в схеме [расширения](openxr.md#roadmap) , представляют собой экспериментальные расширения поставщиков, которые можно просмотреть для сбора отзывов.  Эти расширения предназначены только для устройств разработчика и будут удалены при поставке настоящего расширения.

Если вы заинтересованы в ознакомлении с доступными расширениями `MSFT_preview`, выполните следующие действия, чтобы обновить проект:
...
Чтобы активировать расширения предварительного просмотра на целевом компьютере HoloLens 2 или на настольном ПК:
