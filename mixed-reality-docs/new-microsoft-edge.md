...
1. Установите **Центр обновления Windows** для накопительного обновления 2020-07 для Windows 10 версии 2004 и установите обновление, если оно доступно.
2. Запустите **редактор реестра**.
3. Перейдите к HKEY_CURRENT_USER \Софтваре\микрософт\виндовс\куррентверсион\холографик\преаллокатевиртуалмониторс
4. Измените значение DWORD с 1 (значение по умолчанию) на 0 (ноль).
    * TRUE — 1
    * FALSE-0

Виртуальные мониторы теперь будут выделяться при попытке запустить приложение Win32 в Windows Mixed Reality вместо предварительного выделения. Чтобы сбросить это и повторно включить предварительное выделение виртуального монитора, верните значение DWORD в 1.
