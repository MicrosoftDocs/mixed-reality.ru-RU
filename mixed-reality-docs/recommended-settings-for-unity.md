...
В порядке для платформы Windows Mixed Reality для оптимизации стабильности голограмма он полагается на буфер глубины точные и соответствует любой голограммы готовый для просмотра на экране. Таким образом с помощью управления доступом на буфер глубины, очень важно при визуализации цвета, формировать глубины. В Unity большинство непрозрачный или TransparentCutout материалы будут отображаться глубины, по умолчанию, но прозрачным и текстовые объекты обычно не будет отображаться глубины несмотря на то, что это шейдера зависимых и т. д. 

При использовании смешанной реальности Toolkit стандартный шейдера для подготовки к просмотру глубина для прозрачных объектов:
1) Выберите tranparent материал, который использует стандартный MRTK шейдера, а затем откройте окно редактора инспектора
2) Задайте **режим отрисовки** для **Custom** задайте **режим** для **Transparent** и наконец установите **глубины записи**для **на**
