...
> [!NOTE]
> Кроме того, может быть полезно для установки [сервер кэша](https://docs.unity3d.com/Manual/CacheServer.html), особенно для проектов Unity с большим объемом ресурсов (за исключением файлов сценариев) или постоянно изменение сцены и ресурсы. При открытии проекта, Unity сохраняет соответствующих средств в формат внутреннего кэша на компьютере разработчика. Необходимо повторно импортировать и таким образом повторно обработаны при изменении элементов. Этот процесс можно выполнить один раз и сохраняется в кэше сервера и поэтому совместно с другими разработчиками, чтобы сэкономить время, а не каждому разработчику, обработки повторно импортировать новые изменения локально.
