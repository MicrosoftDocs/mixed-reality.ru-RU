...
В разделе **изменить** > **Параметры проекта** > **качество** > задайте значение **по умолчанию** , щелкнув стрелку вниз, чтобы получить **очень низкий** уровень качества.

### <a name="lighting-settings"></a>Параметры освещения

Как и параметры сцены качества, важно установить оптимальные параметры освещения для приложения смешанной реальности. В Unity параметр освещения, который обычно имеет наибольшее воздействие на производительность сцены, является **глобальным освещения в режиме реального времени**. Это можно отключить, перейдя в **окно** > **подготовки** > **параметры освещения** > **глобально освещения в режиме реального времени**.
...
1) Откройте **Параметры проигрывателя XR** (перейдите в раздел **Правка** > **параметры проекта** > параметры **проигрывателя** > **XR**).
2) Установите флажок **включить совместное использование буфера глубины** в пакетах **SDK для виртуальной реальности** > расширение **Windows Mixed Reality** (флажок**поддерживается в Virtual Reality** ).
...
Unity имеет устаревшую поддержку серверной части сценариев .NET и, таким словами, рекомендует разработчикам использовать **IL2CPP** для сборок UWP Visual Studio. Хотя это дает различные преимущества, создание решения Visual Studio из Unity для **Il2CPP** может выполняться значительно медленнее, чем старый метод .NET. Поэтому настоятельно рекомендуется следовать рекомендациям по созданию **IL2CPP** , чтобы сэкономить время итерации разработки.
...
1) Выберите " **изменить** > **Параметры проекта** > страницу **проигрывателя** "
...
|----------|----------|----------|
|  On  |  Нет  |  Показывать экран-заставку Unity по умолчанию в течение 5 секунд или до загрузки приложения, в зависимости от того, какое из этих значений больше. |
|  On  |  Настраиваемое  |  Отображение пользовательского экрана-заставки в течение 5 секунд или до загрузки приложения, в зависимости от того, что больше. |
|  Off  |  Нет  |  Показывать прозрачный черный (Nothing) до загрузки приложения. |
|  Off  |  Настраиваемое  |  Отображение пользовательского экрана-заставки в течение 5 секунд или до загрузки приложения, в зависимости от того, что больше. |
