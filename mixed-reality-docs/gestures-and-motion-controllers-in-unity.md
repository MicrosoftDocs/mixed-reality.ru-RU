...
Существует два основных способа выполнения действий с вашим взглядом [в Unity](gaze-in-unity.md), [жестами](gestures.md) и контроллерами [движения](motion-controllers.md) в HoloLens и иммерсивное ХМД. Доступ к данным для обоих источников пространственных данных осуществляется через одни и те же API в Unity.
...
Идентификаторы кнопок и осей в таблице ниже поддерживаются в диспетчере ввода Unity для контроллеров движения Windows Mixed Reality с помощью входных API-интерфейсов *input. OnButton и AXIS* , а столбец "Windows MR" относится к свойствам, доступным в Тип *интерактионсаурцестате* . Каждый из этих API подробно описан в следующих разделах.
...
<td> Аналоговый стик <i>X (слева:-1,0, справа: 1,0)</i> </td><td> Ось 1 </td><td> Ось 4 </td><td> Сумбстиккпоситион. x</td>
</tr><tr>
<td> Аналоговый стик <i>по оси Y (Top:-1,0, низ: 1,0)</i> </td><td> Ось 2 </td><td> Ось 5 </td><td> Сумбстиккпоситион. y</td>
</tr><tr>
<td> Нажатый аналоговый стик </td><td> Кнопка 8 </td><td> Кнопка 9 </td><td> сумбстиккпрессед</td>
</tr><tr>
<td> Сенсорная <i>панель X (слева:-1,0, справа: 1,0)</i> </td><td> Ось 17 * </td><td> Ось 19 * </td><td> Таучпадпоситион. x</td>
</tr><tr>
<td> Сенсорная <i>панель Y (верхняя:-1,0, Нижняя: 1,0)</i> </td><td> Ось 18 * </td><td> Ось 20 * </td><td> Таучпадпоситион. y</td>
</tr><tr>
...
**Имен** *UnityEngine*, *UnityEngine. XR*<br>
**Типы**: *Входные данные*, *XR. Инпуттраккинг*
...
Например, чтобы связать кнопку триггера левого контроллера движения с действием Отправить, перейдите в меню **правка > параметры проекта > входные данные** в Unity и разверните свойства раздела отправить в разделе оси. Измените свойство **почтовые кнопки** или кнопка с положительным нажатием, чтобы прочесть **игровую кнопку 14**, например:
...
**Имен** *UnityEngine. XR. WSA. input*<br>
...
* Какие [виды нажатия](motion-controllers.md) происходят (выбор/меню, распечатка, Сенсорная панель или аналоговый стик)
...
* *Интерактионсаурцелост* (станет неактивным)
* *Интерактионсаурцепрессед* (Коснитесь, нажатие кнопки или "Select" распространенная)
* *Интерактионсаурцерелеасед* (окончание касания, кнопка отжата или конец "Select" распространенная)
* *Интерактионсаурцеупдатед* (перемещает или иным образом изменяет некоторое состояние)
...
* В момент, когда произошло нажатие жеста или контроллера, эта головная программа может быть **использована для определения** того, что именно [облаками](gaze.md) пользователь:
...
Приложение может также распознать составные жесты более высокого уровня для пространственных источников входных данных, коснуться, удерживать, манипуляции и жесты навигации. Эти составные жесты можно распознавать как [в](gestures.md) движении, так и в [контроллерах движения](motion-controllers.md) с помощью GestureRecognizer.
...
По умолчанию *GestureRecognizer* не отслеживает входные данные до тех пор, пока не будет вызван *старткаптурингжестурес ()* . Возможно, что событие жеста может быть создано после вызова *стопкаптурингжестурес ()* , если входные данные были выполнены до того, как был обработан *стопкаптурингжестурес ()* . *GestureRecognizer* будет запоминать, была ли она включена или отключена во время превиау фрейма, в котором он действительно выполнялся, и таким образом можно запускать и прекращать мониторинг жестов на основе целевых объектов взгляда на этот кадр.
...
* **Используйте *скорость* контроллера вместо расположения**. В ноябре-обновлении Windows мы предоставили изменение в поведении, когда в [позиционированном состоянии отслеживания "приближено](motion-controllers.md#controller-tracking-state)". В этом состоянии информация о скорости контроллера будет по-прежнему отображаться до тех пор, пока мы считаем, что он имеет высокую точность, что часто бывает дольше, чем в положении.
* **Внедрение *угловой скорости* контроллера**. Эта логика хранится в `throwing.cs` файле `GetThrownObjectVelAngVel` в статическом методе в связанном выше пакете.
...
## <a name="follow-along-with-tutorials"></a>Пройдите вместе с учебниками
