...
Существует два основных способа выполнения действий с вашим [взглядом в Unity](gaze-in-unity.md), [жестами](gaze-and-commit.md#composite-gestures) и [контроллерами движения](motion-controllers.md) в HoloLens и иммерсивное ХМД. Доступ к данным для обоих источников пространственных данных осуществляется через одни и те же API в Unity.
...
<td> Аналоговый стик X <i>(слева:-1,0, Right: 1,0)</i> </td><td> Ось 1 </td><td> Ось 4 </td><td> Сумбстиккпоситион. x</td>
</tr><tr>
...
<td> Сенсорная панель X <i>(левая:-1,0, Right: 1,0)</i> </td><td> Ось 17 * </td><td> Ось 19 * </td><td> Таучпадпоситион. x</td>
</tr><tr>
<td> Сенсорная панель Y <i>(начало:-1,0, Нижняя: 1,0)</i> </td><td> Ось 18 * </td><td> Ось 20 * </td><td> Таучпадпоситион. y</td>
</tr><tr>
...
* В момент, когда произошло нажатие жеста или контроллера, эта головная программа может быть **использована для определения** того, что именно [облаками](gaze-and-commit.md) пользователь:
...
Приложение может также распознать составные жесты более высокого уровня для пространственных источников входных данных, коснуться, удерживать, манипуляции и жесты навигации. Эти составные жесты можно распознавать как в движении, [так и в](gaze-and-commit.md#composite-gestures) [контроллерах движения](motion-controllers.md) с помощью GestureRecognizer.
...
![модель контроллера движения и](images/motioncontrollertest-teleport-1000px.png) телепереноса<br>
...
* **Внедрение *угловой скорости* контроллера**. Эта логика содержится в файле `throwing.cs` в статическом методе `GetThrownObjectVelAngVel` в связанном выше пакете:
