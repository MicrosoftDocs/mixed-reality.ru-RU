...
Контроллеры движения — это [аппаратные аксессуары](hardware-accessories.md) , которые позволяют пользователям выполнять действия в смешанной реальности. Преимущество контроллеров движения над [жестами](gestures.md) заключается в том, что контроллеры имеют точную точку в пространстве, что позволяет детально регулировать взаимодействие с цифровыми объектами. Для впечатляющих головных телефонов Windows Mixed Reality контроллеры движения являются основным способом, которым пользователи будут выполнять действия в своем мире.
...
* Посетите страницу [Установка инструментов](install-the-tools.md) для предпочтительных версий Windows, Unity и т. д. для разработки смешанной реальности.
...
7. В нижней части списка отобразится "контроллер движения — левый" или "контроллер движения — справа". Выберите для связывания. Контроллер будет вибрировало при подключении.
   ...
   *Выберите "контроллер движения" для связывания; Если имеется несколько экземпляров, выберите один из них в нижней части списка.*
   
8. Вы увидите, что контроллер отображается в параметрах Bluetooth в **категории "мышь, клавиатура, & перо"** в качестве подключенной. На этом этапе вы можете получить обновление встроенного по — см. [следующий раздел](motion-controllers.md#updating-controller-firmware).
9. Повторное подключение аккумулятора.
10. Повторите шаги 1-9 для второго контроллера.

После успешного связывания обоих контроллеров параметры должны выглядеть следующим образом: **"мышь, клавиатура, & перо"** . 

   ![Подключенные контроллеры движения](images/450px-motion-controller-connected-300px.png)<br>
   *Подключенные контроллеры движения*

Если после связывания контроллеры отключены, их состояние будет отображаться как парные. Если контроллеры постоянно находятся в разделе "другие устройства", может быть выполнено только частичное связывание, и их необходимо выполнить снова, чтобы обеспечить работоспособность контроллера.

### <a name="updating-controller-firmware"></a>Обновление встроенного по контроллера

* Если к компьютеру подключена иммерсивное головной телефон, а новое встроенное по контроллера доступно, встроенное по будет автоматически отправлено на контроллеры движения при следующем включении. Обновления встроенного по контроллера обозначаются с помощью шаблона ИНДИКАТОРов освещения на круговом движении и занимает 1-2 минут.
* После завершения обновления встроенного по контроллер будет перезагружен и повторно подключен. Оба контроллера должны быть подключены прямо сейчас. 
    
    ![Подключенные контроллеры](images/cyk-connected-300px.jpg)<br>
    *Контроллеры, подключенные в параметрах Bluetooth*
...
Windows Mixed Reality поддерживает две ключевые модели для взаимодействия, **взгляда** и фиксации, а затем зафиксируйте и зафиксировать:
* С помощью **взгляда и фиксации**пользователи нацелены на объект с помощью [взгляда](gaze.md) , а затем выбирают объекты с помощью воздушного воздуха, планшета, щелчка или голоса.
...
Приложения, поддерживающие указание с помощью контроллеров движения, должны также включать возможности взаимодействия на основе взгляда, когда это возможно, чтобы дать пользователям возможность выбирать используемые устройства ввода.

### <a name="managing-recoil-when-pointing"></a>Управление переобмоткой при наведении

При использовании контроллеров движения для указания и фиксации пользователи будут использовать контроллер для целевого объекта, а затем предпринять действия, изменив его триггер. Пользователи, получившие триггер вигораусли, могут в итоге надежде, что контроллер будет выше в конце своего запроса триггера, чем планировалось.
...
Для лучшего представления этих контроллеров существует два вида экземпляров, которые можно исследовать для каждого источника взаимодействия, **захвата** и **указателя**.
...
* **Расположение захвата**: Карманный центроид при естественном удержании контроллера, настраивается влево или вправо для центрирования места внутри захвата. В контроллере движения Windows Mixed Reality эта отправка обычно соответствует кнопке "расположить".
...
## <a name="interactions-low-level-spatial-input"></a>Журнал Пространственные входные данные низкого уровня
...
* **Главная** — это специальное системное действие, используемое для возврата в меню "Пуск". Аналогично нажатию клавиши Windows на клавиатуре или кнопки Xbox на контроллере Xbox. Вы можете зайти домой, нажав кнопку Windows на контроллере движения. Обратите внимание, что вы также можете вернуться к началу, выполнив команду "Привет Кортане, Go домой". Приложения не могут реагировать на действия, относящиеся к домашним, так как они обрабатываются системой.

## <a name="composite-gestures-high-level-spatial-input"></a>Составные жесты: Высокоуровневый пространственный ввод

Как [жесты](gestures.md) , так и контроллеры движения можно отслеживать с течением времени для обнаружения общего набора **[составных жестов](gestures.md#composite-gestures)** высокого уровня. Это позволяет приложению обнаруживать жесты касания,  **удержания**, манипуляции  и **навигации** , если пользователи в итоге используют руки или контроллеры.
...
Эти модели должны подготавливаться к просмотру на **захвате** контроллера, так как источник модели соответствует этой точке в физическом мире. При подготовке к просмотру моделей контроллеров можно райкаст в сцену из **указателя**«a», который представляет луч, на котором пользователи естественным образом задаются, учитывая физическую структуру контроллера.

Дополнительные сведения о динамической загрузке моделей контроллеров в Unity см. в разделе [Подготовка модели контроллера движения в](gestures-and-motion-controllers-in-unity.md#rendering-the-motion-controller-model-in-unity) Unity.
...
Встроенное по контроллера движения является частью драйвера гарнитуры и будет автоматически обновляться при необходимости подключения. Обновление встроенного по обычно занимает 1-2 минут в зависимости от радиосигнала Bluetooth и качества связи. В редких случаях обновление встроенного по контроллера может занять до 10 минут, что может указывать на плохое подключение Bluetooth или радиопомехи. Сведения об устранении проблем с подключением см. в статье рекомендации по [Bluetooth в](https://docs.microsoft.com/windows/mixed-reality/enthusiast-guide/troubleshooting-windows-mixed-reality#bluetooth-best-practices) статье о энтузиастах. После обновления встроенного по контроллеры перезагружаются и подключаются к основному компьютеру (вы можете заметить, что индикаторы будут освещены для отслеживания). Если обновление встроенного по прервано (например, при потере питания контроллеров), будет предпринята попытка повторного выполнения в следующий раз, когда контроллеры будут включены.

### <a name="how-i-can-check-battery-level"></a>Как можно проверить уровень заряда батареи?

На [домашней странице Windows Mixed Reality](navigating-the-windows-mixed-reality-home.md)можно включить контроллер, чтобы увидеть уровень его аккумулятора на противоположной стороне виртуальной модели. Отсутствует индикатор физического аккумулятора.
...
* [Как работает внутреннее отслеживание](https://docs.microsoft.com/windows/mixed-reality/enthusiast-guide/tracking-system)
