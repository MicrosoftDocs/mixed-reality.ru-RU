...
10. На странице *быстрого запуска* службы *API Luis* перейдите к первому шагу, *Возьмите ключи*и щелкните **ключи** (это можно также сделать, щелкнув синие клавиши-гиперссылки, расположенные в меню навигации службы, обозначенном значок ключа). При этом будут раскрыты *ключи*службы.
...
18. Повторите описанный выше процесс, пока все фразы продолжительностью не помечаются сущностями. Если вам нужна помощь, см. рисунок ниже.
    ...
    > [!TIP]
    > При выборе слов для пометки в качестве сущностей:
...
26. В разделе *ресурсы и ключи*:
    ...
    2.  Вы увидите элемент **Starter_Key** ниже, игнорируя его.
...
5.  Перейдите в раздел **файл > параметры сборки** и убедитесь в том, что:
    ...
    6. Сохраните сцену и добавьте ее в сборку.
        ...
        3. Откройте созданную папку **сцены** , а затем в текстовом поле *имя файла*введите **MR_LuisScene**, а затем нажмите кнопку **сохранить**.
...
2.  После создания  папки Scripts (скрипты) дважды щелкните ее, чтобы открыть. Затем в этой папке щелкните правой кнопкой мыши, **создайте > C# скрипт**. Назовите сценарий *микрофонеманажер*. 
...
2.  Щелкните правой кнопкой мыши в  папке Scripts и выберите **создать C# > скрипт**. Назовите сценарий *луисманажер*. 
...
1.  Дважды щелкните папку **Scripts** , чтобы открыть ее. 
2.  Щелкните правой кнопкой мыши в  папке Scripts и выберите **создать C# > скрипт**. Назовите *поведение*сценария. 
3.  Дважды щелкните скрипт, чтобы открыть его в *Visual Studio*.
...
3.  Установите флажок " **проекты Unity C#**  " (удобно для просмотра и отладки кода после создания проекта UWP.
...
6.  Щелкните **выбрать папку** , чтобы начать сборку в этом расположении.
    ...
    ![Создать папку](images/AzureLabs-Lab3-44.png)
    ![сборок выбор папки сборок](images/AzureLabs-Lab3-45.png)
...
>[!IMPORTANT] 
> Эта глава чрезвычайно важна и может потребоваться интератед несколько раз, так как она поможет улучшить точность службы LUIS: Убедитесь, что вы выполнили это действие.
...
![Проверить изображение](images/AzureLabs-Lab3-49.png)
удаления метки фразы продолжительностью![](images/AzureLabs-Lab3-50.png)
