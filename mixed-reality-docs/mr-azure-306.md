...
![окончательный продукт](images/AzureLabs-Lab6-00.png)
![. Запуск окончательного продукта — запуск](images/AzureLabs-Lab6-01.png)
...
2.  Настройка и тестирование иммерсивного наушников смешанной реальности.
    ...
    > [!NOTE]
    > Для этого курса **не** потребуется использовать контроллеры движения. Если вам нужна поддержка настройки иммерсивного головного телефона, щелкните ссылку, [чтобы настроить Windows Mixed Reality](https://support.microsoft.com/en-au/help/4043101/windows-10-set-up-windows-mixed-reality).
...
2.  Теперь необходимо указать имя проекта Unity, вставить **\_MR 360VideoStreaming.** . Убедитесь, что для типа проекта задано значение **3D**. Задайте для расположения нужное расположение (Помните, что ближе к корневым каталогам лучше). Затем нажмите кнопку **создать проект**.
...
7. На этой панели необходимо проверить несколько параметров:
    ...
    1.  На вкладке **другие параметры** выполните следующие действия.
        ...
        1.  **Создание сценариев** **Версия среды выполнения** должна быть стабильной (эквивалент .NET 3,5).
...
4.  В **иерархии**выберите **основной объект Camera** и обновите следующие компоненты:
    ...
    1.  **Преобразует**
        ...
        1.  Расположение = **X**: 0, **Y**: 0, **Z**: 0.

        2. Поворот = **X**: 0, **Y**: 0, **Z**: 0.

        3. Масштаб по **оси X**: 1, **Y**: 1, **Z**: 1.

    2.  **Камера**

        1. **Снять флаги**: Сплошной цвет.

        2.  **Обтравочные плоскости**: Близ 0,1, далеко: 6.
...
2.  Щелкните правой кнопкой мыши в **проекта** панели **создать* *C\# Script**. Присвойте скрипту имя « **взгляд**».
...
16. Выберите новый материал **буттонматериал** (под названием **буттонматериал 1**) и в инспекторещелкните окно цвета **албедо** . Появится всплывающее окно, где можно выбрать другой цвет (выберите любой из них), а затем закрыть всплывающее окно. Материал будет его собственным экземпляром и отличаться от исходного.
