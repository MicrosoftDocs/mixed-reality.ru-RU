...
Чтобы добавить привязку к международному обзвонку, вызовите метод AddComponent<WorldAnchor>() для объекта Game с преобразованием, которое необходимо привязать в реальном мире.
...
Иногда привязки размещаются немедленно. В этом случае для этого свойства-расположения привязки будет задано значение true, если метод AddComponent<WorldAnchor>() возвращает. В результате событие Онтраккингчанжед не будет активировано. Чистый шаблон будет вызывать обработчик Онтраккингчанжед с начальным состоянием после присоединения привязки.
