...
Эмулятор HoloLens позволяет тестировать голографические приложения на обычном компьютере без физического устройства HoloLens. Он поставляется с набором средств для разработки HoloLens. Этот эмулятор использует виртуальную машину Hyper-V. Данные, которые поступают от человека или из окружающей среды и которые обычно считываются датчиками HoloLens, имитируются с помощью клавиатуры, мыши или контроллера Xbox. Вам не нужно изменять приложения, чтобы запускать их в эмуляторе, так как они не получают информацию о том, что выполняются не на реальном устройстве HoloLens.

Если вы нацелены на разработку приложений или игр для настольных компьютеров с Windows Mixed Reality и гарнитурой смешанной реальности (VR), см. подробнее о [симуляторе Windows Mixed Reality](using-the-windows-mixed-reality-simulator.md), который позволяет имитировать гарнитуры для классических приложений.


## <a name="installing-the-hololens-emulator"></a>Установка эмулятора HoloLens
Скачайте эмулятор HoloLens и шаблоны голографических проектов.

Доступные версии: 
* [эмулятор HoloLens 2 и шаблоны голографических проектов](https://go.microsoft.com/fwlink/?linkid=2087187);
...
Если ваша система отвечает указанным выше требованиям, для успешной установки эмулятора **проверьте наличие в системе компонента Hyper-V**. Для этого откройте панель управления -> "Программы" -> "Программы и компоненты" -> "Включение или отключение компонентов Windows" и убедитесь, что компонент Hyper-V выбран.
...
1. Загрузите решение приложения в Visual Studio.
    >[!NOTE]
    >Если вы используете Unity, обычным образом скомпилируйте проект из Unity и загрузите созданное решение в Visual Studio.
2. Для эмулятора HoloLens (1-го поколения) должна быть выбрана платформа **x86**. Для эмулятора HoloLens 2 должна быть выбрана платформа **x86** или **x64**.
...
* **Перемещение руки для прокрутки** — перемещайте мышь вверх-вниз, удерживая нажатыми клавишу ALT и правую кнопку мыши, или на контроллере Xbox перемещайте вверх-вниз левый джойстик, удерживая нажатыми правый триггер и клавишу A.
* **Перемещение и ориентация руки** (только в эмуляторе HoloLens 2) — перемещайте мышь вверх, вниз, влево и вправо, удерживая нажатой клавишу ALT, чтобы перемещать руку, или нажимайте клавиши со стрелками и Q или E для вращения и наклона руки.  На контроллере Xbox перемещайте левый аналоговый стик, удерживая нажатым левый или правый бампер, чтобы перемещать руку влево, вправо, вперед или назад, перемещайте правый аналоговый стик для поворота руки или перемещайте Dpad вверх или вниз для подъема и опускания руки.
...
Справа от главного окна размещается панель инструментов эмулятора. На панели инструментов расположены следующие кнопки:
...
Панель управления имитированием позволяет просматривать текущее положение и ориентацию имитируемого человека и имитируемых устройств ввода.  Также вы здесь можете настроить имитируемые устройства ввода, например отобразить или скрыть одну или обе руки, или выбрать для управления этими устройствами клавиатуру, мышь и игровой контроллер.
...
* Наведите указатель мыши на элемент управления или поле, чтобы увидеть всплывающую подсказку с элементами управления на клавиатуре, мыши и игровом контроллере, которые соответствуют этому элементу управления.
...
* Чтобы управлять направлением взгляда, щелкните значок канцелярской кнопки в разделе "Глаза".  Он действует так же, как удержание клавиши Y на клавиатуре.
...
* Чтобы изменить скорость перемещения или вращения имитируемого человека или имитируемых устройств ввода в ответ на манипуляции с клавиатурой, мышью или игровым контроллером, щелкните значок шестеренки рядом с элементом Input settings (Параметры ввода) и переместите ползунки.
* По умолчанию имитируемый человек и имитируемое устройство ввода управляются с клавиатуры.  Чтобы переключить ввод с клавиатуры компьютера на HoloLens, снимите флажок Use keyboard for simulation (Использовать клавиатуру для имитирования).  Этот параметр переключается с помощью клавиши F4.
* Если панель имитирования уже отображается, клавиша F8 перемещает в нее фокус клавиатуры.
* Чтобы открепить панель имитирования от окна эмулятора, нажмите кнопку в нижней части панели или клавишу F9 на клавиатуре.  При закрытии окна или повторном нажатии клавиши F9 это окно снова отобразится в эмуляторе.
...
При запуске эмулятора вы увидите окно, которое отображает ОС HoloLens.
...
* ![Значок закрытия](images/emulator-close.png) **Закрыть**. Закрывает эмулятор.
* ![Значок свертывания](images/emulator-minimize.png) **Свернуть**. Сворачивает окно эмулятора.
* ![Значок пользовательского ввода](images/emulator-control.png) **Пользовательский ввод**. Клавиатура и мышь используются для имитирования [пользовательского ввода для эмулятора](#basic-emulator-input).
...
Вкладка "Имитирование" отображает текущее состояние имитированных датчиков, с которыми работает ОС HoloLens в эмуляторе. Наведя курсор на любое значение на этой вкладке, вы увидите всплывающую подсказку с методами для управления этим значением.
...
В эмуляторе вы увидите только визуализируемые голограммы, но не обстановку имитируемых комнат. Это важное отличие от настоящего устройства HoloLens, в котором эти две среды смешиваются друг с другом. Если вы хотите увидеть имитируемую комнату в эмуляторе HoloLens, следует включить в приложении отображение сетки пространственного сопоставления для нужной сцены.
...
* См. [полный список требований к системе](#hololens-emulator-system-requirements) выше в этой статье.
* Убедитесь также, что в системе включена функция Hyper-V.
...
Если в Visual Studio список целевых объектов для отладки пустой (или в нем есть только действие "Пуск") и вы уже выполнили все описанные выше действия, сделайте следующее:
